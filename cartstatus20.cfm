...
<cfparam name="prev_cart1" default="" />
<cfset prev_cart = prev_cart1 />
...
<cfset upsindia = "yellow" />
  ...
  <cfquery datasource="gemssql" name="s">
     ...
     cartid not in (select distinct cartid from cartforothers where (shipdate is null or shipdate > #createodbcdate(dateadd('d',-url.no_of_days,createdate(datepart('yyyy',Now()),datepart('m',Now()),datepart('d',Now()))))#) and supplier like 'Ace%' or supplier in ('harish', 'vijay', 'arpit', 'GR', 'anup', 'kashish', 'royal','pawan')   )  and 
	  <cfelseif url.show is "abandoned"> status = 'new'  and <cfelseif url.show is "backoffice"> status = 'new' and paymode <>  'null' and 
	  </cfif> 
	  (indate > #createodbcdate(dateadd('d',-1*url.no_of_days,createdate(datepart('yyyy',Now()),datepart('m',Now()),datepart('d',Now()))))#  or paymode in ('authphone','auth.net','pp','chck','ichck','goog','ccavenu','arb1','arb2')) order by
  <cfif url.show is "abandoned">
  buyer desc, 
  <cfelse> 
...
<cfset prev_cart = listappend(prev_cart,cartid) />
...
<cfcatch type="any">
  ...
  <!---<cfcase value="ebs,ichk,paypal,wellscc,wellspho-ne,check,google,ccav"> --->
  <cfcase value="#application.paymode_shipped#" >
    <tr class="done">
      <cftry>
          <cfset ordercount = ordercount + 1 />
          <cfset dt = dt + cost />
...
<tr <cfif couponcode is 'regwsale' or couponcode is 'saved'> style="color:red"</cfif>>
        ...
        <td ><a href="itemsell.cfm?cartid=#cartid#,#prev_cart#&couponcode=#couponcode#&paymode=&amp;next_cart=#prev_cart#" title="see list"  >#decimalformat(cost)#</a></td><td>#shippingdetails#</td>
...
<a href="admin.cfm">Please log in...</a>
