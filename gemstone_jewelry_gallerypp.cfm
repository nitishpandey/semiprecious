...
Select  orderbuy, disporder, newitem, CAT, subcat, datetaken,  NameID,     price  , saleprice, status, wholesaleprice, Description, inventory, storage, thumblink, imagelink,
   buylink ,  orderprice from (SELECT 0 as orderbuy, storage, thumblink, imagelink, disporder, newitem, CAT, subcat, datetaken,   NameID,     price  , saleprice, status, wholesaleprice, Description, inventory,
...
<cfif jewelrysize NEQ ""> and size>19</cfif> 
  ...
  <CFIF advancedsearch is not "">
    UNION SELECT 1 as orderbuy, disporder,newitem, CAT, storage, thumblink, imagelink, subcat, datetaken, NameID,       price    , saleprice, status, wholesaleprice, Description, inventory, buylink, case status when 3 then saleprice else price end  as orderprice from Items where (inventory>#invstatus#)
    <cfif advancedsearch is "">
      <CFIF category NEQ "ALL">
        and (CAT = '#category#'             )
      </CFIF>
  ...
  <CFIF advancedsearch is not "">
    UNION SELECT 2 as orderbuy, disporder, newitem, CAT, storage, thumblink,imagelink, subcat, datetaken, NameID,       price    , saleprice, status, wholesaleprice, Description, inventory, buylink,case status when 3 then saleprice else price end as orderprice from Items where (inventory>#invstatus#)
    <cfif advancedsearch is "">
      <CFIF category NEQ "ALL">
        and (CAT = '#category#' 
     )
  ...
  <CFIF advancedsearch3 is not "">
    UNION SELECT 3 as orderbuy,  disporder, newitem, CAT, storage, thumblink, imagelink, subcat, datetaken, NameID,       price    , saleprice, status, wholesaleprice, Description, inventory, buylink, case status when 3 then saleprice else price end as orderprice from Items where (inventory>#invstatus# )
    <cfif advancedsearch is "">
      <CFIF category NEQ "ALL">
        and (CAT = '#category#' 
         )
  ...
  <CFIF advancedsearch1 is not "">
    UNION SELECT 4 as orderbuy,disporder, newitem, CAT, storage, thumblink, imagelink, subcat, datetaken, NameID,      price    , saleprice, status, wholesaleprice, Description, inventory, buylink, case status when 3 then saleprice else price end as orderprice from Items where (inventory>#invstatus# )
    <cfif advancedsearch is "">
      <CFIF category NEQ "ALL">
        and (CAT = '#category#' 
      )
...
<cfif getlist.storage EQ 'Pippin'>
 <a href='jewelry_item.cfm/#item_array[columns]#.htm' > 
              <img src ="#thumblink#" longdesc="http://www.semiprecious.com/longdesc.cfm?newitem=#newitem#" border='0' bordercolor='black'  name="img#newitem#" title="#subcat# #category#"></a>
<CFELSE>
 <a href='jewelry_item.cfm/#item_array[columns]#.htm' > 
...
<br /><a href='jewelry_item.cfm/#item_array[column]#.htm' ><font color="##CC3300">Buy</font></a></cfdefaultcase>
				  </cfswitch><div  align="justify"  class="listdescr">#left(description, 200)#. 
 <cfif not isdefined("session.bulkbuyer.id")> Wholesale click <a href='wholesale_jewelry.cfm'>here</a></cfif> </div> </td>	
...
<tr><td bgcolor="white" style="border: #light# 1px dotted;border-bottom: #light# 0px dotted">		<a href="jewelry_item.cfm"  >By Item No.</a><br>
</td></tr>		 
