...
Select top 25 sum([inventory]) as stock, stone from catsubcatinstock  group by stone order by stock desc) x order by stone		
	...
	<cfquery name="stonelist" datasource="gemssql">
		select distinct stone as subcat2, inventory  from CatSubCatInStock  where cat='#category#' and inventory>0 order by  subcat2
	</cfquery>
...
#subcat2# <cfif len(subcat2) LT 10>#lcase(categ)#</cfif><font size-1>(#inventory#)</font><cfif style neq "">-#style#</cfif></a>
		</td> 
        </tr>
    <cfelse>
