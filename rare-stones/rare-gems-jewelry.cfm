<cftry>
	...
	<cfquery name="contentbypage" datasource="gemssql" cachedwithin="#createtimespan(0,10,0,0)#">
	...
	<cftry>
		...
		<cfcatch type="Any">
			<cfinvoke method="sendmail"  component="backing_beans.mailer" >
				...
				<cfinvokeargument name="mailcontent" value="#cfcatch.details#|#cfcatch.message#" />
			</cfinvoke>
			<CFINCLUDE TEMPLATE="../header.cfm" />
			<b>
				Sorry, there has been a database error, please call us at 1 512 589 9009 to inform us.
			</b>
	...
	<cftry>
		<cfif (category neq "ALL") and (category neq "")>
			<cfset p = category>
			<cfquery DATASOURCE="gemssql"  name="getlist" cachedwithin="#createtimespan(0,10,0,0)#">
...
and  cat <> 'ornaments' and cat <>'bags'  and cat <>'beads' and cat <>'healing' and cat <>'gems' and stone <>'bulk lots'  and stone <>'fabric' and stone<>'silk thread' group by stone order by <cfif sortby eq 'stone'> stone<cfelse> inventory desc</cfif>
		...
		<cfcatch type="database">
			<cfinvoke method="sendmail"  component="backing_beans.mailer" >
				<cfinvokeargument name="mailto" value="stacyanup@gmail.com	" />
				<cfinvokeargument name="subject" value="error" />
				<cfinvokeargument name="mailcontent" value="#cfcatch.details#|#cfcatch.message# at rare-stones/rare-gems-jewelry.cfm" />
			</cfinvoke>
			<CFINCLUDE TEMPLATE="../header.cfm" />
			<b>
				Sorry, there has been a database error, please call us at 1 512 589 9009 to inform us.
...
<CFINCLUDE TEMPLATE="/header.cfm">
<!--- already contains the body element, a div top level and then a div with id container1 --->
<!--- Since mobile or not deciphering is happening in header code, now we have to do some display logic here --->
<cfif mobile>
	<cfset displaycolumns = 2 />
</cfif>
<!-- <cfoutput>#displaycolumns#</cfoutput> --->
<cfcache action="cache" timespan="#createtimespan(0,10,0,0)#">
	<!--
		<cfscript>
		writeoutput("Page from cache created at ");
		writeoutput(createodbctime(now()));
		</cfscript>-->
	<div id="container2">
		<h1 align="center" class="form_heading">
			SELECT A GEMSTONE FROM OUR VAST RANGE FOR
			<cfoutput>
				#ucase(P)#
			</cfoutput>
		</h1>
		<!--- let's keep an aesthetic gap between h1 and the upcoming table --->
		<p>
			&nbsp;
		</p>
		...
		<table id="container3" cellspacing="0" cellpadding="0" border="0">
			...
			<CFLOOP QUERY="Getlist">
				...
				<cfoutput> <td width="5%" align="center"> <img src=/images/stones/gemstone-thumbs/#replace(lcase(subcat2)," ","",'all')#-n.jpg width=20 height=20<!--- alt="#subcat2# gemstone thumbnail" --->> </td> <td width="20%"> <a href='/#replace(lcase(subcat2)," ","",'all')#<cfif category neq "ALL">_#category#</cfif>.cfm' style="font-family:Arial,size:10; Helvetica, sans-serif;color:##333333"> #ucase(subcat2)# <cfif category is "ALL">COLLECTION<cfelse>#ucase(category)#</cfif> </a> (#inventory#) </td> </cfoutput> <cfif columns EQ displaycolumns> <cfset rowsdisplayed = rowsdisplayed + 1 /> </tr> <tr> <td colspan="<cfoutput>#2*displaycolumns#</cfoutput>"> &nbsp; </td> </tr> <CFSET columns=1 /> <cfelse> <CFSET columns=columns + 1 /> </cfif>
			</CFLOOP>
			<cfif columns neq 1>
				<cfoutput>
					<!---- the last box in the row was not filled before comng out of query loop--->
					<td coslpan="#evaluate(displaycolumns - columns+1)#">
						&nbsp;
					</td>
					</tr>
				</cfoutput>
			...
			<cfoutput>
				<cfif pagedescription neq "" or session.mail is application.admin_email>
					<Tr valign="middle">
						<td valign="middle" style="color:##666600;" class="rowb" colspan=#displaycolumns#>
							<b>
							...
							<cfif session.mail is application.admin_email>
								<a href="http://www.semiprecious.com/admintools/contentadder.cfm?pagename=#replace(currentfile,'/','')#">
									Content Edit
								</a>
							</cfif>
						</td>
