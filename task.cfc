...
<cfcomponent displayName="task controller" hint="contols all activities realted to tasks">
  ...
  <cffunction access="public" name="update" output="true"  displayname="Update status" hint="update to ack, aborted or completed">
    ...
    update task set status = '#Arguments.status#', closedon = #Now()# where tdtoi     = '#Arguments.tdtoi#' and tby = '#Session.apptt#' 
    </cfquery>
  </cffunction>
  ...
  <cffunction access="public" name="getcartissues" output="false" returntype="query" displayname="View Cart Issues (initiation only)">
    ...
    <cfquery name="details" datasource="sptm" username="nitish" password="rangeela" >
    	SELECT task.tdtoi, task.tby, status,action, pdc, taskdetails.details  FROM task, taskdetails  WHERE task_type= 'cart_issue' and taskdetails.subject = '#Arguments.cartid#' and task.tdtoi = taskdetails.tdtoi and task.tby = taskdetails.tby order by status
    </cfquery>
    <cfreturn details>
  </cffunction>
