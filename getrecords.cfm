...
Select top 500 weight, size, totalqtysold, itemnumber as optcount, grouping, COLor, orderonrequest, getdate() as lastbought, case 	when inventory>0 then '0' else '1'
  ...
  Select top 500 weight,  size, totalqtysold, itemnumber as optcount, grouping, COLor, orderonrequest, case 
...
when restockdate is null then datetaken
   ...
   where <cfif session.country is 'india'>storage like 'IN%' and </cfif> (inventory>-1)  <cfif subcat neq "cubic zirconia"> and (storage not like 'CM%')</cfif>
<CFIF cgi.server_name contains "wholesale" or session.bulkbuyer.id neq ""> AND WHOLESALEPRICE >0 and (storage not like 'CM%')
<CFELSE>
and price>0 
</CFIF>
