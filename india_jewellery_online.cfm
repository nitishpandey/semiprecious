...
<cfoutput>
  <TITLE><cfif ringsize neq "">Rings #ringsize# </cfif><cfif cgi.server_name contains 'wholesale'>Wholesale </cfif>#titleCase(subcat)# #style#<cfif color neq ""> #titleCase(color)#</cfif> <cfif groupname neq ""> #titlecase(groupname)#</cfif> #titlecase(p)# Gemstone #titlecase(metatitle)# </TITLE>
  <meta name="keywords" content=<cfif cgi.server_name contains 'wholesale'> Wholesale</cfif>#color# #subcat#, #groupname#, Gemstone, #p#, #metakeywords#">
...
<cfquery dbtype="query" name="getlist"> 
 ...
 order by  score desc 
	...
	<cfdefaultcase>
<cftry>
		<cfquery datasource="gemssql" name="getlist" cachedwithin="#createtimespan(0,0,0,0)#" >
		    <cfinclude template="/includes/getrecordsitemidsindia.cfm" />
		</cfquery>
...
<cfset getlist.recordcount = 0 />
SORRY, AN ERROR HAS OCCURED, PLEASE EMAIL US - cs@semiprecious.com
</cfcatch>
...
<cfoutput>
	  ...
	  <meta name="description" content="<cfif metadescription neq ""><cfif getlist.recordcount GT 0><cfif start GTE getlist.recordcount><cfset start = 1 />
	</cfif><CFLOOP QUERY="Getlist" startrow="#start#" endrow="#evaluate(start+1)#">#xmlformat(getlist2.description)#</cfloop>.</cfif>#metadescription#<cfelse><cfif cgi.server_name contains 'wholesale'>Wholesale</cfif> #subcat# #style# #p# gem stone jewelry, retail and wholesale. 
  <cfif getlist.recordcount GT 0><CFLOOP QUERY="Getlist" startrow="#start#" endrow="#evaluate(start+5)#">#xmlformat(getlist2.description)#
  </cfloop></cfif></cfif>">
</cfoutput>
...
<div align="left" style="margin-left:5px;">  
		<b><font size=+1 color=purple>Showing:</b> <cfoutput>#Getlist.recordcount# #titlecase(color)# #titlecase(subcat)#<cfif groupname neq ""> #titlecase(groupname)#</cfif> #titlecase(url.style)# <cfif len(category) GT 3> #titlecase(category)#</cfif><cfif pricegreater neq "0"> Over #format(pricegreater)#</cfif><cfif priceless neq ""> Under #format(priceless)#</cfif>
		</cfoutput></font>&nbsp; 
	<cfif subcat neq "" or groupname neq "" or pricegreater neq "0" or priceless neq ""><a href=/<cfoutput>#GetFileFromPath(GetBaseTemplatePath()
)#</cfoutput>
>Clear Filters</a></cfif>
<br /><img src="/images/filter.jpg" style="display:inline" width="18px" /> <cfif category is "all" or subcat eq "" or  color  eq ""> Filter:</cfif> <cfinclude template="/includes/searchdropdowns.cfm" /> <cfif subcat neq "" or groupname neq "" or pricegreater neq "0" or priceless neq ""><a href=/<cfoutput>#GetFileFromPath(GetBaseTemplatePath()
)#</cfoutput>
>Clear Filters</a></cfif>
</div>  
...
<cfset quick_add = session.quick_add />
  ...
  <!--- Paging links --->
    <CFINCLUDE TEMPLATE="/includes/gallerypaging.cfm">
		...
		<cfif Not len(session.name)>
					<br><br>
          <cfoutput>
       		<div align="left" style="width:100%; border-collapse:collapse; border:thin">
                  <span style="font-family:verdana san serif,color:purple;font-size:11px;">
...
<div align="center" id="cart_summary" style="z-index:6;"></div>
  ...
  --->
    ...
    <tr>
		<td>
    		<table id="container1" width="900px" cellspacing="0" cellpadding="0" border="0"  style="padding:0 2px 0 2px" align="center">
    			<tr>
                    <td valign="top">
