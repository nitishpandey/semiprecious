/* this code does not generate a new cartid. that is the responsibility of the calling code. it also does not add items into db with the new cartid since we know that a visit to cart table will do that and visit to cart page is inevitable. finally the new cart id 's entry in cartstatus table is made by the code in cartmonitor */
<cfcomponent displayname="cart_from_cookie" hint="called during session restart inside session_start.cfc if cookie identifier is found">

	<cffunction name="init" access="public" description="can set up cart for a provided cart id; ">
		<cfargument required="true" displayname="non zero cart id to use"  name="_cartid" />
		<cfset session.desc_flag = 0 />
		<cfset session.check1 =  arguments._cartid />
		<cfquery datasource="gemssql" name="currentcart">
	           SELECT * FROM buyingrecord WHERE buyingrecord.cartid = #arguments._cartid#
        </cfquery>
		...
		<cfif currentcart.recordcount >
			...
			<cfif recover_cart>
				...
				<cfquery datasource="gemssql" name="notpaid">
	             		  SELECT cartid,cost, status, paymode, buyer FROM cartstatus WHERE  cartid = '#arguments._cartid#' AND paymode='null' and paymode is not null
	    		 </cfquery>
				<cfif notpaid.recordcount  >
					<!--- not paid so worthwhile to recover the cart --->
					<cfquery datasource="gemssql" name="confirmedstatuscheck">
				               SELECT cartid, status, paymode, buyer FROM cartstatus WHERE cost>0 and cartid = '#arguments._cartid#' AND paymode='null' and paymode is not null
				            </cfquery>
					...
					<cfelse>
						...
						<cfinvoke component="cartcontrol.cartmonitor" method="new_cartid" >
							<cfinvokeargument name="reuse_cartid" value="#arguments._cartid#" />
						</cfinvoke>
					</cfif>
	...
	<cffunction name="setup_summary" access="private" description="depends on session;sets up total qty and value in session object;we should keep this in session.cart instead">
		<cftry>
			<cfif session.cartitem[1][1]>
				<cfloop from="1" to="#arraylen(session.cartitem)#" index="counter" >
					<cfset session.grandtotal = session.grandtotal + session.cartitem[counter][3]*session.cartitem[counter][4] />
			...
			<!---
				...
				--->
			<cfcatch type="any">
				<cfmail to="anupstacy@gmail.com" from="cs@semiprecious.com" subject="Error getting cart from cookie id " server="#application.mail_server#" type="html">
