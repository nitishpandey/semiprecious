...
Select top 400 weight, size, totalqtysold, itemnumber as optcount, grouping, COLor, orderonrequest, getdate() as lastbought, case 	when inventory>-5 then '0' else '1'
   ...
   items  </cfif>
     where <cfif session.country is 'india'>storage like 'IN%' and </cfif> storage='Angela' and inventory>-5 <CFIF category NEQ "ALL">
  ...
  Select top 400 weight,  size, totalqtysold, itemnumber as optcount, grouping, COLor, orderonrequest, case 
   when lastbought is null then restockdate 
when restockdate is null then datetaken
else lastbought end as  lastbought, case when inventory>-5 then '0' else '1'
		end as instock, 
	0+
	<!---<cfif advancedsearch1 is not "">
		...
		<cfif advancedsearch3 is not "">
		 case 
		  ...
		  </cfif> ---> 1		 as orderbuy,  disporder, disporderwhole, newitem, CAT, style, subcat,subcat2, datetaken,   NameID,  price, basecost, saleprice, status, wholesaleprice, Description, inventory, storage, thumblink, imagelink,
   buylink , case status when 3 then saleprice else price end as orderprice from 
    <cfif (url.sortorder contains "totalqtysold")> itemspopular<cfelse>
   items  </cfif>
   <cfif groupname neq ""> inner join  (select itemid from itemsbyGroup  where groupname='#groupname#') igr on newitem=itemid </cfif> 
   where <cfif session.country is 'india'>storage like 'IN%' and </cfif> (inventory>-5 or orderonrequest=1)  <cfif subcat neq "cubic zirconia"> and (storage not like 'CM%')</cfif>
<CFIF cgi.server_name contains "wholesale" or session.bulkbuyer.id neq ""> AND WHOLESALEPRICE >0 and (storage not like 'CM%')
<CFELSE>
and price>0 
</CFIF>
...
subcat like '%#advancedsearch1#%' <cfif advancedsearch2 is not "">or subcat like '%#advancedsearch2#%'</cfif> <cfif advancedsearch3 is not "">or subcat like '%#advancedsearch3#%'</cfif>
  ...
  <cfif ringsize neq ""> and newitem in (select  itemid from options, items where cat='rings' and options.itemid=items.newitem  and options.description='#ringsize#' )  </cfif>
   ...
   order by orderbuy 
    <cfelse>
    order by  status, instock, <cfoutput>#url.sortorder#</cfoutput>    
 </cfif>
</cfoutput>
