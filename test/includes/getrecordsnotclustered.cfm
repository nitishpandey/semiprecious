...
Select grouping, color, getdate() as lastbought, case 	when inventory>0 then '0' else '1'
   ...
   buylink ,  orderprice from (SELECT top 4 0 as orderbuy,  grouping, color, lastbought, restockdate, disporder, newitem, CAT, style, subcat, subcat2, datetaken,   NameID,     price  , saleprice, status, wholesaleprice, Description, inventory, storage, thumblink, imagelink,
...
when restockdate is null then datetaken
  ...
  items
   ...
   where  newitem not in (select distinct itemid from styles) and (inventory>-3)  <cfif subcat neq "cubic zirconia"> and (storage not like 'CM%')</cfif>
<CFIF cgi.server_name IS "www.wholesale-gemstone-jewelry.com" or session.bulkbuyer.id neq ""> AND WHOLESALEPRICE >0 and (storage not like 'CM%')
<CFELSE>
and price>0 
</CFIF>
...
subcat like '%#advancedsearch1#%' <cfif advancedsearch2 is not "">or subcat like '%#advancedsearch2#%'</cfif> <cfif advancedsearch3 is not "">or subcat like '%#advancedsearch3#%'</cfif>
  ...
  <cfif ringsize neq ""> and newitem in (select  itemid from options, items where cat='rings' and options.itemid=items.newitem  and options.description='#ringsize#' )  </cfif>
   ...
   order by instock, orderbuy 
    <cfelse>
    order by status, instock, <cfif url.sortorder is "lastbought"> disporder desc <cfelse><cfoutput>#url.sortorder#</cfoutput> </cfif>    
 </cfif>
</cfoutput>