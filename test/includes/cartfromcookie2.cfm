      ...
      <cfif goahead>
		     ...
		     <cfset session.check2 =  currentcart.email />
		  		  	   <cfif  currentcart.email contains "@"   >
					  	   	    <cfif len(session.bulkbuyer.id)>
											...
											<cfif ws_loggedin.recordcount or isdefined("url.cartid")><!--- cartid in url could be from non www redirection --->
		...
		<cfset session.check5 =  1 />
				<cfquery datasource="gemssql" name="notpaid">
			         ...
			         <cfif notpaid.recordcount  ><!--- not paid so worthwhile to recover the cart --->
						  <cfquery datasource="gemssql" name="confirmedstatuscheck">
							...
							<cfelse>
					      	...
					      	<cfloop  query="currentcart"> <!--- need to replace below process with invocation of add to cartmonitor 20 Jun, Nitish--->
                            ...
                            <!---      <cfquery datasource="gemssql" name="catcheck">
						            ...
						            --->        	<cfcatch type="any"><cfdump var="#cfcatch.tagcontext#" />
