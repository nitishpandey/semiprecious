<cftry>
	...
	<cfparam name="BULKBUYER.FIVEHUNDREDDISCOUNT"  default="10" />
	<CFQUERY DATASOURCE="GEMSSQL" NAME="COUNTRYCODE">
    SELECT CODE FROM LTBCOUNTRYcode WHERE COUNTRY ='#form.x_ship_to_country#'
</CFQUERY>
	<!--- <CFIF cgi.HTTP_REFERER DOES NOT CONTAIN 'CART' >
		...
		--->
	<cfcatch type="any">
		<cfset COUNTRYCODE = querynew('code') />
		<cfset r = queryaddrow(COUNTRYCODE) />
		<cfset success = querysetcell(COUNTRYCODE, 'code','US') />
	</cfcatch>
</cftry>
...
<cflock scope="session" throwontimeout="no" timeout="30"  type="exclusive">
	...
	<!--- not sure why should we have to cfpaaram, but doing as a quick back up for now. nitish --->
	<cfparam name="x_address" default="">
	<!--- by anup --->
	...
	<cfif isdefined("form.clientcreditcard")>
		...
		<cfset x_invoice_num=#form.x_invoice_num#>
         ...
         <cfset nvp = createobject("component","nvp") />

		<cfset responseStruct = nvp.getNVPResponse(country: COUNTRYCODE.CODE, state: x_state, forms_data: form ) />
		...
		<cfif  responseStruct.Ack is "Success" || responseStruct.Ack is "SuccessWithWarning">
			...
			<cfset transid = responseStruct.TRANSACTIONID  />
			<!---  Replace By Ankur response_array[7] [5/2/2012]--->
			<!---<cfset invoiceid = response_array[8] />--->
			<!--- This Line Commented By Ankur [5/2/2012]--->
   ...
   On relay back from payment, code line 319  <cfdump var="#cfcatch#" />
			...
			<cftry>
				...
				<cfif tflag is 'none'>
					<!--- this means relay backs will not switch a retail buyer to a wholesale buyer --->
...
<br>
        ...
        <table border="0" style="width:600px;border-bottom:1px red solid;" bordercolordark="##666600">
        <tr >
...
<tr><td colspan="2"><!-- Start Bizrate POS Code -->
...
<script type="text/javascript" src="https://eval.bizrate.com/js/pos_35112.js">
</script></td></tr>
</table>
...
#err_mess#,<cfdump var='#form#' />
	...
	<cftry>
		...
		<cfif not len(session.bulkbuyer.id)>
		 ...
		 Select creditcode from credit with (nolock) where creditcode ='#couponcode#'
				...
				<cfif credit.recordcount NEQ 1>
					<h2>
						INVALID CREDIT COUPON, PLEASE GO BACK AND RE-ENTER CODE or email: cs@semiprecious.com
					</h2>
				</cfif>
		...
		<HTML>
			<HEAD>
				<TITLE>
					Select Secure Checkout to buy Jewelry
				</TITLE>
	...
	function()
	{
	 ...
	 $('#sameas').click(
	  function(){
		...
		<cfabort /> </cfif> </cflock> </div>
		<table  id="container1" style="width:60%;margin-top:38px;border-bottom:2px gray solid;" border="0" align="center"  >
			<cftry >
	...
	<cfif session.cartitem[k][5]>
						...
						<cfoutput query="qprice">
							...
							<cfif  (qprice.inventory LTE 0) and qprice.orderonrequest >
								...
								<cfset aodtext ='
									...
									This design will be made against your order. This will take about 15 days and your jewelry will ship in about 3 weeks from Austin.</span>' />
							</cfif>
							<!---	<td>
			    ...
			    update buyingrecord set inventoryatsale = #Variables.inventoryatsale# where cartid = #session.cartid#
                           and optionid = #session.cartitem[k][5]# and itemid = #session.cartitem[k][1]#		      </cfquery>
							...
							<cfelse>
								...
								<cfset session.cartitem[k][4] = 0 />
								<!--- cannot simply delete it as we are looping and array size will diminish --->
								<cfquery datasource="gemssql" name="updatcart">
				...
				<cfoutput>
					...
					<cfif total>
						...
						<tr>
							<td colspan="2" bgcolor="##799ea3"   height="24px" style="color:white" >
						...
						#giftmsg# for cart #session.cartid#
							...
							<tr>
								<td colspan="1" style="padding:4px;font:16px;font-family:courier;border:red 2px dashed;color:black;">
									<a style="text-decoration:none;" name="payment">
										Msg with Gift
									</a>
									:
									<cfoutput>
								...
								<td align="right">
									...
									<!--- value beign added and shown as shipping #format(GiftWrap)# --->
								</td>
							</tr>
						<cfelse>
							<cfset gw = "">
				...
				<cfif isdefined("session.giftid")>
					<cfif session.giftamt GT 0>
						...
						<tr>
							<td colspan="2" align="center" class="headerfontsmall">
								GIFT AMOUNT REDUCED FROM TOTAL. GIFTID NOT VALID IN FUTURE.
								<br>
								If you wish to shop more using the same giftid you must
								<a href="havegiftcoupon.cfm">
									re-validate
								</a>
								the certificate.
								<br>
								If not then please make balance payment
								<strong>
									if any
								</strong>
								and mail us the giftid and this cartid.
							</td>
						</tr>
					</cfif>
				</cfif>
							...
							<td colspan="2" align="right"  >
								...
								<cfif amount GT 1999>
									<div  class="green_band_ticked">
										...
										Volume Discount of
										<cfoutput>
											#session.bulkbuyer.twothousanddiscount#
										</cfoutput>
										% Applied
									</div>
								<cfelseif amount GT 999>
									<div  class="green_band_ticked">
										...
										Volume Discount of
										<cfoutput>
											#session.bulkbuyer.thousanddiscount#
										</cfoutput>
										% Applied
									</div>
								<cfelseif amount GT 499>
									<div  class="green_band_ticked">
										...
										Volume discount of
										<cfoutput>
											#session.bulkbuyer.fivehundreddiscount#
										</cfoutput>
										% Applied
									</div>
								</cfif>
								<cfif amount GT 399>
									<div  class="green_band" style="padding-bottom:4px">
										<cfoutput>
											<cfif amount GT 1450>
												Shop for #format(2000)# get #session.bulkbuyer.twothousanddiscount#% off
											<cfelseif amount GT 740>
												Shop for #format(1000)# get #session.bulkbuyer.thousanddiscount#% off
											<cfelseif amount GT 499>
				...
				<cfelse>
					<tr align="center" >
						<cfif (len(confirm_mail) and not isdefined("url.save")  and not isdefined("tpsale")) or len(session.bulkbuyer.id)  >
							...
							<td align="right">
								<cfoutput>
									#format(evaluate(total+coupon_saving))#
								</cfoutput>
							</td>
					</tr>
...
<font color="maroon"><cftry> <cfinvoke component="invitationandcredit" method="reversecredit"  returnvariable="q"> <cfinvokeargument name="mail" value="#confirm_mail#" /> </cfinvoke> <cfcatch type="any"> <cfmail to="anup@semiprecious.com" from="cs@semiprecious.com" server="MailA40" subject="At confirmation Could not reverse discount "  type="html">
				...
				<cfif isdefined("wholefault")>
					...
					<tr >
						...
						<td  align="center" valign="middle" style="padding:2px;background-color:#FFE4EC;border:2px solid #A50A25;color:#A50A25;font-size:2em;" >
							Wholesale purchase requires minimum order of
							<cfoutput>
								#format(minimum)#.
								<a href="#session.gallery#?#session.filter#" class="gray_url">
									Click Here to Shop for More Stuff
								</a>
							</cfoutput>
							&nbsp; If you wish to shop less then please visit
					...
					<tr>
						<td  colspan="2">
							<font face="Arial, Helvetica, sans-serif" size="+1">
								To shop for less than the minimum wholesale amount you may switch to retail mode and make purchases at retail prices.
								<a href="/switchtoretailmode.cfm" class="side_link">
									Click here to switch and go to gallery
								</a>
							</font>
							.
						</td>
	...
	<cfelse> <cfif isdefined("tpsale")> <cfset modtps = 'y'> <cfelse> <cfset modtps = 'n'> </cfif> <cfoutput> <cfif ((coupon_saving and (not loyalty_discount) and couponflag)  and isdefined("tpsale") ) or len(session.bulkbuyer.id) > <tr class="" > <td colspan="5" align="right">Items Total:</td> <td align="right">#format(evaluate(coupon_saving+amount-shipping))#</td> </tr> <cfif coupon_saving> <tr class="" > <td colspan="5" align="right">#coupon_msg#. Your Total Savings/Discount:</td> <td align="center" class="error1" style="text-align:right;">#format(coupon_saving)#</td> </tr> </cfif> <cfelseif gift_value GT 0> <cfset topay = amount - shipping /> <!---  <tr class="" >
			<td colspan="5" align="right">Total:</td>
			<td align="center" style="text-align:right;">#format(topay)#</td>
			</tr>---> <cfif gift_value GT topay> <cfset gift_value = topay > </cfif> <cfset topay = topay - gift_value /> <cfset bbdf = topay/(topay+gift_value) /> <cfset amount = topay + shipping /> <cfquery datasource="gemssql" >
                	update buyingrecord  set rate = #bbdf#*rate where cartid = #session.cartid#
                </cfquery> <tr class="" > <td colspan="1" class="error1" align="right">#coupon_msg#. Value:</td> <td align="center" class="error1" style="text-align:right;">#format(gift_value)#</td> </tr> <cfelseif gift_value LT 0 and cgi.server_name does not contain 'laptop'>
				<tr > <td colspan="1" class="error1" align="right">[#coupon_msg#] </td> <td align="center" class="error1" style="text-align:right;">&nbsp;</td> </tr> </cfif> <cfif cgi.server_name does not contain 'wholesale'  and (session.x_ship_to_state eq 'TX' or session.x_ship_to_state eq 'texas')>
					<tr> <td  align="right" ><cfset salestax = (amount-shipping)*0.0825 /><cfset amount=salestax+amount> Sales Tax:</td> <td align="right" >#dollarformat(salestax)#</td> </tr> </cfif>
					<tr class="" > <td  align="right">Final Items Total:</td> <td align="right">#format(amount-shipping)#</td> </tr>
					<tr> <td  align="right" >Shipping &amp; Handling:</td> <td align="right">#format(shipping)#</td> </tr> <!--- Grand total line --->
					<tr  > <td  align='right' class="tableheader1" style="padding:5px;"><cfif application.live_support and 0 > <div style="display:inline;border:1px ridge ##0A0 ;margin-top:4px;padding:2px 4px" align=center> <a class="login_link" onClick="open_live_support()" href="javascript://"><img style="text-decoration:none;" src="/images/callus.png" height="20px" VALIGN="center" border=0 />&nbsp;<span style="position:relative;bottom:4px">Online Help</span></a> </div> </cfif> <cfif confirm_mail is application.admin_email> <cfset amount=amount  -admindiscount> </cfif> <font size="+1" color="gray">Grand total:</font></td> <td align="right" style="padding:1px;border:1px 0 1px 0 solid gray"><strong>#format(amount)#</strong></td> </tr> <!--- End grand total line ---> </table> <!--- End totals table ---> </cfoutput> <cfif Giftwrap neq "0"> <cfset gw = "G"> <cfelse> <cfset gw = ""> </cfif> <cfif isdefined("url.save")> <cfinvoke component="invitationandcredit" method="reversecredit"  returnvariable="q"> </cfinvoke> <div align="center"> <div style="width:740px;text-align:center;font-size:12px;"> <div style="width:738px;text-align:center;background-color:#666600;color:white;"> Your Cart Contents have been Saved. </div> <cfif len(confirm_mail) and confirm_mail contains '@' > <cfset mailto = confirm_mail> <cfmail server="MailA40" bcc="anup@semiprecious.com" to="#mailto#" failto="anup@semiprecious.com" from="service@semiprecious.com" subject="Shop later at semiprecious.com"  type="html">
 ...
 <tr> <td align="center" bgcolor="#FFFFFF" class="row2b"> <table style="margin:auto"  border="0" cellpadding="0" > <tr> <td bgcolor="#FFFFFF" align="center"> <!--- this does not work if server time is not giving correct conversion to UTC. So check GMT time and tehc conversion that the code is giving. Adjust server time manually to cater for the difference ---> <cfinclude template="/authorizenet/simtoembed.cfm" /> </td> <cfset session.amount=#decimalformat(amount)#> </tr> </table> </td> </tr> <cfelse> <!---   <tr>
			<td  height="20"  align="center" class="greenbutton"><img src="images/secure_lock_2.gif" width="16" height="17" style="display:inline;" border="0" vspace="0" hspace="0" align="absmiddle"> <b> MOST SECURE AND BEST CREDIT CARD PAYMENT SYSTEMS </b></td>
			</tr>---> <tr> <td align="center" bgcolor="#FFFFFF" > <div style="position:relative;"> <table    border="0" cellpadding="0" > <tr> <td align=right valign=top  > <span style="margin:auto;width:740px">Your Cart ID is: <cfoutput> <b> #session.cartid# </b> <br>You can access it any time from:</span><br> #cgi.SERVER_NAME#/myorder </cfoutput></td><td>&nbsp;</td></tr> <tr> <td style="margin:auto;width:720px" align=left valign=top colspan="2"> <cfif amount LT 0.1 and couponcode neq ""> <div id="noneedtopay" style="position:absolute;height:200px;font-color:#993333 ;font-weight:bold;font-size:16;font-face:arial"> YOUR ORDER IS COMPLETE, NOTHING FURTHER TO DO, THANK YOU</div><p>&nbsp;<p> <cfmail to="semiprecioushelp@gmail.com"  server="MailA40" subject="Order Redeemed by Coupon" from="cs@semiprecious.com">
					 FYI: Order <cfoutput>#session.cartid#</cfoutput> has redeemed a coupon, not needing payment by CC
     			 </cfmail> </cfif> <cfif amount LT 1 and amount GT 0.1 and couponcode neq ""> <div id="noneedtopay" style="position:absolute;height:200px;font-color:#993333 ;font-weight:bold;font-size:16;font-face:arial"> YOUR ORDER IS COMPLETE, NOTHING FURTHER TO DO, WE WILL TAKE CARE OF THE TINY AMOUNT LEFT IN THIS ORDER. THANK YOU</div><p>&nbsp;<p> <cfmail to="semiprecioushelp@gmail.com"  server="MailA40" subject="Order Redeemed by Coupon" from="cs@semiprecious.com">
					 FYI: Order <cfoutput>#session.cartid#</cfoutput> has redeemed a coupon, not needing payment by CC
     			         </cfmail> </cfif><p>&nbsp;<p> <ul class="arrow_ul"> 
                     <li style="position:relative;left:200px;"><h3><a style="text-decoration:none;" name="payment"><strong>Click on PayPal Logo to Pay Using </strong> <cfoutput>
					  <form name="_xclick" action="https://www.paypal.com/cgi-bin/webscr" method="post"  style="display:inline;"> <input type="hidden" name="cmd" value="_xclick">
					...
					<input type="hidden" name="currency_code" value="USD"> <input type="hidden" name="item_name" value='#session.cartid##sh##Gw#' />
					<input type="hidden" name="invoice" value='#session.cartid#' /> <input type="hidden" name="amount" value='#decimalformat(amount)#'>
					<!--- TODO: when we get ssl certificate make https --->
					 <input width="70px" type="image" src="https://www.semiprecious.com/images/paypal_logo.gif" border="0" name="submit" alt="Make payments with PayPal - it's fast, free and secure!">
					  ...
					  <input type="hidden" name="custom" value="#session.cartid#"> <!--- Newly added variables--End ---> </form>
							</cfoutput> 
</h3> <!---As a security feature authorize.net button is <span style="color:red">valid for next 3 minutes</span> only.---> 
</li> 
<!---
<li><h3><a style="text-decoration:none;" name="payment"><strong>Or Fill Details Below To Pay Using Credit Card </strong>
			
</h3> !---As a security feature authorize.net button is <span style="color:red">valid for next 3 minutes</span> only. 
</li> --->
</ul></td> </tr>
				<tr> <td valign=top align="right" style="width:580px">
			<!---	<cfinclude template="/authorizenet/simtoembed.cfm" />--->
				</td>
				<td>&nbsp;</td></tr>
				<!---
				<tr><td> <cfset session.amount=#decimalformat(amount)#> <p><br><p> <b><font color=#3333FF><p>MORE PAYMENT OPTIONS BELOW:</font></b> &nbsp; <p> Pay Through Paypal (Takes Checks Also) <cfoutput> <form name="_xclick" action="https://www.paypal.com/cgi-bin/webscr" method="post"  style="display:inline;"> <input type="hidden" name="cmd" value="_xclick"> <cfif cgi.server_name neq "www.ornamentscollect.com"> <input type="hidden" name="business" value="anup@semiprecious.com"> <cfelse> <input type="hidden" name="business" value="lizzy@ornamentscollect.com"> </cfif> <input type="hidden" name="currency_code" value="USD"> <input type="hidden" name="item_name" value='#session.cartid##sh##Gw#' /> <input type="hidden" name="invoice" value='#session.cartid#' /> <input type="hidden" name="amount" value='#decimalformat(amount)#'> <input width="150px" type="image" src="https://www.semiprecious.com/images/paypal_logo.gif" border="0" name="submit" alt="Make payments with PayPal - it's fast, free and secure!"> <!--- Newly added variables--Start ---> <input type="hidden" name="custom" value="#session.cartid#"> <!--- Newly added variables--End ---> </form> <p> <!---  <cfset GoogleCart =  createObject("component","googleCheckOut/GoogleCheckout")>
			#googleCart.CreateForm()#---> </cfoutput></td><td>&nbsp;</td> </tr>--->
			 </table> </div> </td> </tr> </cfif>
			 <tr> <td align="center" class="grayplacard">Any questions or concerns? Please <a target="contactus" href="contactus.cfm">Contact us</a></td> </tr>
			  <tr> <td align="center" ><cfinclude template="/mainfooter.cfm" /></td> </tr> </table> </cfif> <!--moved up by Anup--->
			  <cfset session.confirmed_cartid = session.cartid />
			  <CFQUERY DATASOURCE="gemssql" >
					update cartstatus set buyer = '#confirm_mail#', agentcode='#agentcode#' where cartid  = '#session.cartid#'
				</cfquery> <!--moved up by Anup---> <!---<cfset session.cartid = 0 />---> <!--- now the cartid will not increase on refresh but otherwise it will ---> <!--- handmade ---> </cfif> </cfif> <cfcatch type="any"> <cfmail to="anup@semiprecious.com"  server="MailA40" subject="Error at confirmation" from="cs@semiprecious.com">
