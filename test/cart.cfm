...
<!---mysql---><cfquery datasource="gemssql">
...
<cfelse>
		...
		<div class="form_heading">Hey! Welcome back</div>
			Please log in using <a class="reg_price" href="/simpleloginform.cfm">'login'</a> link on the header of this page.
			After you login you can also recover your earlier cart items into your current cart in a single click.
		...
		<img src="/images/activate_help.jpg">
		 --><br />
		<br /><br /><span class="reg_price">If you want to continue without logging in <cfoutput><a class="error" href="/cart.cfm?country=#session.address.country_code#&cartid=#session.cartid#">click here</a></cfoutput></span>
				</div>
...
<cfset emptyitems = 0>
 ...
 <cfelse>
   <cfif isdefined("variables.itemsincart") >
    <cfset showheaderjs = 1 />
    </cfif>
 </cfif>
...
if(inString=='Proceed to Checkout')
{
	...
	<cfelse>
<!---- TODO: Nitish Nov 2014 make this https when ssl certification is installed --->
...
self.document.cartform.submit();
	...
	texas = 1
		...
		if(inString=='Save Cart'){
			...
			<cfelse>
/* TODO: Nitish: make https when ssl is installed */
...
<option value="">Select Shipping</option>
 ...
 <img src="/images/moreshopping.gif" style="margin-top:2px;border:2px gray ridge;width:120px;cursor:pointer" onClick="javascript:goProcess('buy more')" > &nbsp;
				...
				<tr><td colspan="2" bgcolor="##799ea3" style="color:##FFFFFF" height="24px"><strong>Shipping Details <cfif IsDefined('session.mail') AND session.mail neq ''>(Same As Registered? <input type="checkbox" name="sameas" id="sameas" onClick="fnFillForm(this);" style="background-color:##603082;border-color:##603082">)</cfif></strong>
</td></tr>
		...
		<tr>
                    	...
                    	<td>
                            ...
                            > </div><div id="totalsbox" style="position:relative;floating;left:250px;font-size:16;color:gray;font-weight:bold;">YOUR CART TOTAL <cfoutput>#dollarformat(subtotal)#</cfoutput></div>
                        </td>
                    </tr>
					<tr>
                    	<td>
			  ...
			  <tr cellpadding=3>
           	    <td align="left"><b>Email Id</b> </td>
					...
					<td  align="left" >
					<cfif url.country neq "USA">
              				<cfinput required="true" message="Please provide a valid email id."  type="text" name="email" value="#session.mail#" size="40">
						<cfelse>
         					<cfinput required="true"   type="text" name="email" value="#session.mail#" size="40">
		  			</cfif>
							</td>
              	</tr>
              </cfif>
...
--->
		   <span style="color:red;">Coupon or Gift Code, If Any</td><td> <input name="couponcode" type="text" class="detail" value="" size="12" maxlength="18">   &nbsp;&nbsp;&nbsp; Agent Code, if any: <input name="agentcode" type="text" class="detail" value="" size="12" maxlength="8"><br />
			    ...
			    <!---TODO:Gives   <tr>
	              ...
	              <tr id="giftmsg">
	                ...
	                <td align="left" nowrap ><textarea rows=3 cols=50  name="giftmsg"  onClick="javascript:this.value='';return;" VALUE="" MAXLENGTH="245" ></textarea>
								 ...
								 <tr>
	                <td>
                    	<b>Special Instructions, if Any</b>
                    </td>
	                <td  nowrap >
			  ...
			  <cfif len(session.mail) and session.loyalty_points and session.mail contains '@'>
		                <tr>
		                  <td >  <b>Semiprecious.com Cash Used</b> <a href="viewinvitecredits.cfm" ALT="friends invited for jewelry site">[?]</a>:</td>
