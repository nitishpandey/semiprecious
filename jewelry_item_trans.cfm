...
padding-left:15px;font-weight:700;height:16px;background:url('/images/added.png') no-repeat scroll 0px 0px ##E6F8DD;padding-top:1px;
}
...
<div align="center" >
		  <cfinclude template="/#session.country#/header.cfm" >
	<span  style="height:12px;text-decoration:none;position:relative;left:-40;font-size:14px;" >
		<cfimport   prefix="rootincludes" taglib="/" />
									  <rootincludes:steps step="2" /> 						
	</span>
	<!--- No category --->
	<!--- End no category --->
          ...
          <cfoutput>
            ...
            <table  width="750px" bgcolr="#ff7777" cellspacing="0" cellpadding="0" border="0"  align="center"  >
              ...
              <tr valign="top" >
                ...
                <table border="0" cellpadding="0" cellspacing="0" style="padding:1px" >
	                <tr  valign="middle" >
						...
						<span   style="positio:absolute;top:8px;left:2px;z-index:1;background-color:transparent;" > 
                          &nbsp;<a class="side_link" href='#session.gallery#?#session.filter####newitem#_anchor' style="text-decoration:underline;" >&larr; Back to Gallery</a> 
                      </span> &nbsp;
							<h2 style="display:inline;"><a href="/jewelry_item.cfm?newitem=#newitem#"><cfif cgi.server_name contains 'wholesale'>Wholesale </cfif>#titlecase(details.subcat)# #titlecase(details.grouping)# 
						<cfif details.subcat2 neq ""> #titlecase(details.subcat2)#</cfif> 
						<cfif details.cat neq 'healing'>#titlecase(left(details.cat, len(details.cat)-1))#</cfif></a>
						</h2></cfif><table style="margin-top:1px;width:100%" cellpadding="1px;"><tr><td width="40%" align="center">
						<cfif len(cart_msg)>
								<Div id="cart_summary" style=";margin-top:2px;border:2px solid red;padding:2px 1px 1px 1px;">
									<span style="width:440px;display:inline;background-color:##FDFDFD;;padding:2px;color:red;font-size:1.2em;font-weight:bold;">#cart_msg#&nbsp; </span>&nbsp;
									</div> 
					...
					<img   id="mainimage" style="display:inline;" src ='/images/#folder#/#width#/#imagename#'   alt="unique #subcategory# #category# Jewelry"  border="0" />
						  ...
						  <span id="cart_form" >&nbsp;
						  <span class="greenbutton" style="border-bottom: purple 2px outset; border-left: purple 2px outset; background-color: white; width: 150px; height: 28px; border-top: black 2px outset; top: 8px; font-weight: 800; border-right: purple 2px outset; padding-top: 4px; left: 2px; positio: absolute;">
						                  &nbsp;<a class="" href='#session.gallery#?#session.filter####newitem#_anchor'  >&larr; Back to Gallery</a>&nbsp; 
	                      </span> 
	                      	<cfif not len(session.bulkbuyer.id) or (session.grandtotal GTE session.bulkbuyer.minamt) >
						
							<div style="display:none;"><span id="easy_form">	
							    <form action="/#session.cart#?##checkout" method="get" >
									<cfset country_msg = "Ship to Country" />
									<font size=+1><input type="submit" name="submit" style="border-left:1px solid black;border-top:1px solid black;border-bottom:1px solid purple;border-right:1px solid purple;color:black;background-color:##CAB;display:inline;" value="Select Country  &rarr; Then Click Here" />&nbsp;<cfinclude template="/includes/countries_drop_down.cfm" />
									</font>  
					  			</form>
					  			</span>
							</div>
				...
				style="border-bottom: purple 2px outset; border-left: purple 2px outset; background-color: white; width: 150px; height: 28px; border-top: black 2px outset; top: 8px; font-weight: 800; border-right: purple 2px outset; padding-top: 4px; left: 2px; positio: absolute;"
				onClik="document.getElementById('cart_form').innerHTML=document.getElementById('easy_form').innerHTML;"  onClik="goProcess('cart summary');return;">&nbsp;<a href="/#session.cart#?##checkout" >Start Checkout &rarr;</a>&nbsp;
						  </span>
						  <cfelse>&nbsp;
						  <span class="error2">To Checkout please shop for a minimum of #format(session.bulkbuyer.minamt)#</span>
					  </cfif>
					  </span> 
						  </div>
					...
					<cfif wd.height GT 280 >
					  <img  height="290px" id="mainimage" style="display:inline;" src ='/images/#folder#/#width#/#imagename#'   alt="unique #subcategory# #category# Jewelry"  border="0" />
					  <cfelse>
					<img   id="mainimage" style="display:inline;" src ='/images/#folder#/#width#/#imagename#'   alt="unique #subcategory# #category# Jewelry"  border="0" />
				...
				<td nowrap="" width="" valign="top" align="right" style="background-color:transparen;">
                   ...
                   <!---  <br style="clear:both" >--->
                    ...
                    <cfif details.morepics GT 0 and (not len(width))>
                      <div style="border: 1px gray ridge;padding-top:4px;background-color:#EFEFEF" align="center" > 
...
Approx. number of beads calculate out to:                            #round(evaluate(16*25.4/details.size))#(this is auto calculated based on bead size entered and 16 inch strand length and can be wrong if length of strand is different or size of bead is not along the strand length)
    ...
    <!--- include file to display clustered items, and other category items by Anup--->
    	...
    	select top 5 styles.itemid , 0 as orderbuy, styles.cat,inventory from styles, items where items.newitem=itemid and inventory>0 and styleid =
        ...
        <b><font color="black">YOU MAY ALSO LIKE THESE &dArr;</font></b>
                    ...
                    <CFIF price eq 0 and (len(session.bulkbuyer.id) is 0 )>
                      ...
                      <cfif status eq 0 or status eq 3 >
                        ...
                        <cfif details.basecost >
							 Store Price
							#format(7.18*details.basecost/application.exchangerate)# </cfif>
							<cfif len(session.bulkbuyer.id)>
							<br />Wholesale
                          <cfelse>
                        ...
                        <cfif cgi.server_name neq "63.135.126.234" >
                          <cfif not len(session.bulkbuyer.id)>
							<CFIF  status eq 3>
                              <b><font color="red" >On Sale!</font>:#format(saleprice)#</b><br />
                            </CFIF>
                          </cfif>
...
Sold Out
                           ...
                           <a name="instock" style="text-decoration:none;">
...
<option value="#j#" >#j#</option>
 ...
 <!--- <CFINPUT  type="text" SIZE="1"  id="quantity" Name="quantity" VALUE="#existqty#" REQUIRED="yes" Message="Please enter quantity" VALIDATE="integer" onclick="this.focus();" > --->
							...
							<cfif 0 and len(session.bulkbuyer.id) and optcount><!---- this section disabled for now. when options would be modified use it --->
                                    ...
                                    <cfif valueadd>
		                                      ...
		                                      <!--- if there is a change in price because of this option --->
		                             	         <cfif url.optionid eq opts.optionid>
		                                 	       <option value="#opts.optionid#" selected>#description#(+$#_valueadd#)</option>
		                                    	    <cfelse>
		                                 	       <option value="#opts.optionid#" >#description#(+$#_valueadd#)</option>
		                                	      </cfif>
                                      <cfelse>
		                                    	  <cfif url.optionid eq opts.optionid>
...
<input style="width:100px;height:21px;margin-top:4px;border:1px purple solid;" class="green_bg" type=submit value="Add to Cart" name="Add to cart">

    <!---                        <img src=/images/addtocart.gif d="a2cb"  onClick="javascript:goProcess('add')" >   --->
							<input type="button" class="hidden" id="a2cw" value="Wait..." >
                          </cfif>
                          <!--- Angela --->
						 <cfif not len(width)> <INPUT TYPE="button" id="a2wl" style="width:100px;margin-top:4px;border:1px purple solid;" class="greenbutton" onClick="javascript:goProcess('Add to Wish List')" VALUE="Add To Wish List" >
                          
                          <a style="display:block;margin-top:4px;width:100px;height:20px;border:1px purple solid ;text-decoration:none;" class="greenbutton" accesskey="Z" href="/wish.cfm"  >See Wish List</a>
						  </cfif>
...
Item Not Available.
               ...
               <div class="component_box" style="width:100px;margin-top:4px;">
...
--->        
             ...
             <!--- End right side? --->
			  <!--- 4th column --->
                 <td align="center" >&nbsp;&nbsp;&nbsp;&nbsp;<cfoutput><a href="/#session.cart#" ></cfoutput><img src=/images/add_to_cart_icon.jpg></a><br>&nbsp;&nbsp;&nbsp;&nbsp;<b>CART</b><br><br>
...
Select top #numbershow# itemID from itemsBygroup, items where itemsbygroup.itemid = newitem and GroupName='#findGroup.groupname#' <cfif category neq "All" >and category = '#category#'</cfif> and itemid <> #itemnumber# and inventory>0
                ...
                <cfif groupitems.recordcount GT 0>
                 <cfoutput>More #findgroup.groupname#<br /></cfoutput><cfoutput query="groupitems" ><a href=/detail.cfm?newitem=#itemid#><img src=/images/#category#/thumb/#itemID#.jpg border=0 width=75></a><br />
				...
				Select top 6 newitem, cat from items where subcat='#subcategory#' and cat <>'#details.cat#' and inventory>0 order by newitem desc
				</cfquery>
