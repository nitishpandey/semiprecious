...
Select disporder, newitem, m.CAT, subcat, datetaken,  saleprice, status, wholesaleprice,  inventory, storage, thumblink, imagelink,
   buylink from items m, ltbstones s where s.cat = m.cat and s.stone=m.subcat and
	newitem =(select top 1 newitem from items where cat=m.cat and subcat=m.subcat and inventory>0 and subcat2 is null and (STATUS = 0 OR STATUS=3)   <CFIF CATEGORY EQ "ALL"> AND M.CAT <> 'BEADS' AND M.CAT <> 'HEALING'</CFIF> <cfif category neq "all">and m.cat='#category#'</cfif> <CFIF isdefined("session.bulkbuyer.id")> AND (Storage <> 'pippin') and (storage not like 'CM%')</CFIF>
<cfif cgi.server_name IS "www.wholesale-gemstone-jewelry.com" or cgi.server_name IS "wholesale-gemstone-jewelry.com">
...
<b>#subcat# #category#</b><br>
 ...
 <a href='gem_stone_#cat#.cfm/#item_array[columns]#.htm' >
			...
			<div  class="listdescr">
<a href='gemstone_jewelry_gallery.cfm?category=#category#&subcat=#subcat#'>Show #subcat# #category#</a><p>
<a href='gemstone_jewelry_gallery.cfm?subcat=#subcat#'>Show all #subcat# Jewelry</a>
