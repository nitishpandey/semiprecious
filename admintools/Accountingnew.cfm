...
where buyingrecord.itemid=cast([items].[NEWITEM] as varchar(10)) AND cartstatus.paymode<>'null'   and cartstatus.paymode <> 'emptied' and cartstatus.paymode <> 'activated' AND (cartstatus.couponcode Like 'regwsale%' or cartstatus.couponcode Like 'wsale%')
...
FROM items, (select * from cartstatus where indate >'1/1/07') as cartstatus INNER JOIN buyingrecord ON cartstatus.cartid = buyingrecord.cartid
where buyingrecord.itemid=cast([items].[NEWITEM] as varchar(10)) AND cartstatus.paymode<>'null'   and cartstatus.paymode <> 'emptied' and cartstatus.paymode <> 'activated' AND  (cartstatus.couponcode not Like 'regwsale%' and cartstatus.couponcode not Like 'wsale%' or cartstatus.couponcode is null)
GROUP BY year([indate]),  Month([indate]), items.supplier
HAVING ((month([indate])=#month# AND YEAR([indate])=#YEAR# ) AND ((items.supplier)='Angela'))  order by mnth
</cfquery>
<!--- OC SALES --->
...
SELECT Sum(buyingrecord.cost*quantity) AS Cost, Sum(buyingrecord.rate*quantity) AS Sales, Month([indate]) AS mnth
FROM items, (select * from cartstatus where indate >'1/1/07') as cartstatus INNER JOIN buyingrecord ON cartstatus.cartid = buyingrecord.cartid
where buyingrecord.itemid=cast([items].[NEWITEM] as varchar(10)) AND cartstatus.paymode<>'null'   and cartstatus.paymode <> 'emptied' and cartstatus.paymode <> 'activated'
GROUP BY year([indate]),  Month([indate]), items.supplier
...
SELECT Sum(buyingrecord.cost*quantity) AS Cost, Sum(buyingrecord.rate*quantity) AS Sales, Month([indate]) AS mnth, indate
FROM items, (select * from cartstatus where indate >'1/1/10' ) as cartstatus INNER JOIN buyingrecord ON cartstatus.cartid = buyingrecord.cartid
where buyingrecord.itemid=cast([items].[NEWITEM] as varchar(10)) AND cartstatus.paymode<>'null'    and cartstatus.paymode <> 'emptied' and cartstatus.paymode <> 'activated'
GROUP BY year([indate]), Month([indate])
order by indate
</cfquery>
...
SELECT Sum(buyingrecord.cost*quantity) AS Cost, Sum(buyingrecord.rate*quantity) AS Sales, Month([indate]) AS mnth, indate
FROM items, (select * from cartstatus_archive where indate >'1/1/10') as cartstatus INNER JOIN buyingrecord ON cartstatus.cartid = buyingrecord.cartid
where buyingrecord.itemid=cast([items].[NEWITEM] as varchar(10)) AND cartstatus.paymode<>'null'    and cartstatus.paymode <> 'emptied' and cartstatus.paymode <> 'activated'
...
<body lang="en-US">
<cfoutput><a href="?month=#evaluate(month-1)#">Prev</a>Delighted?<a href="?month=#evaluate(month+1)#">Next</a>
#month#</cfoutput>
