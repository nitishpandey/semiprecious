...
<cfif isdefined('url.suppliername') And isdefined('url.orderby')>
    <div align="right">
      
        <input type="button" value="Export To Excel" onclick="window.open('supplierrestockexcel.cfm?suppliername=#url.suppliername#&orderby=#orderby#');"> 
    </div>
</cfif>
</cfoutput>
...
Select top 1000 newitem, daysactive, t2z, tbyznorm, z2c, zbycnorm, soldbyday, dayspersell, dayspersellnorm,  wishcount,  num_sold,  inventory, weight, lastbought, basecost*0.7 as basecost, cat, '<img src=/images/' + cat + '/thumb/' + cast([newitem] as varchar) + '.jpg height=50>' as itemimage,
 '<A href=/makeup.cfm?itemnumber=' + cast([newitem] as varchar) +'>' +   cast([newitem] as varchar) + '</a>' as item, left([description],100) as description, internalnote, subcat from items right outer join 
 ...
 zoom/(addtocart+1)/c.ZbyCcat as zbycnorm , cast(sold as real)/daysactive as soldbyday, daysactive/(sold+0.01) as dayspersell, (daysactive/(sold+0.01))/dbyscat  as dayspersellnorm from nitish.itemstats i, itemsCat_rating c, items where items.newitem=i.itemid and items.cat=c.CAT and daysactive>0 and sold>0) newstat
  on itemid=newitem 
	right outer join 
( SELECT   count(*) as wishcount, newitem as wishitem FROM wish GROUP BY newitem ) w on wishitem = newitem where  newitem not in (select itemnum from anup.restock) and  ( (wishcount>-1 and inventory<1) ) and  <cfif suppliername is 'harish'> (supplier <>'royal' and supplier not like 'mma%' and supplier <>'anup')
<cfelse> supplier like '%#suppliername#%'</cfif> and 
 (storage not like 'CM%')  <cfif stone neq ""> and subcat = '#stone#'</cfif> order by   #orderby#
...
<table><tr><td>
 <cfif onlyitems is "yes">#newitem#<cfelse> #item#, Wishcount:<font color=green>#wishcount#</font>, num. sold: #num_sold#, instock: #inventory#, <cfif getlist.currentrow LT 500>#itemimage# </cfif>  #subcat#,  #cat#, Wt:#weight# gms Rs:#basecost#, #description#, #internalnote# </cfif>
...
<OPTION>Royal</OPTION>
<OPTION>GR</OPTION>
<OPTION>Pawan Stone Wala </OPTION>
<OPTION>Prashant</OPTION>
<OPTION>Preety</OPTION>
<OPTION>USA</OPTION>
<OPTION>Khanna</OPTION>
<OPTION>Nangeeyan</OPTION>
</SELECT>
