<cftry><cfif session.mail neq application.admin_email>
<cfabort />
</cfif>
...
<CFQUERY DATASOURCE="gemssql" NAME="EditItem">
    SELECT datetaken, g2i, morepics, clustercount,saleprice,disporder,storage,cat,lastbought , rate, supplier,subcat,subcat2,subcat3,style,size, NameID, price,totalqtysold, wholesaleprice,basecost, Description, weight,   shipping, faceted, inventory, color, status, shape, REORDER,internalnote from Items where newitem=#newitem#
</CFQUERY>
...
<cfform  action="updateitem.cfm?" method="post">
 ...
 Existing Item (change to move this item from this cat to another): <input type="hidden" value="#newitem#" name="url_newitem" > 
      ...
      <td> Status :
       ...
       <!---   <cfif session.mail is "acemat@vsnl.com">
        ...
        <Select NAME="g2i">
          ...
          <option value="1" <CFIF #editItem.g2i# is 1>selected</cfif> >Awesome</option>
	      <option value="-1" <CFIF #editItem.g2i# is -1>selected</cfif> >Bad</option>
		   <option value="2" <CFIF #editItem.g2i# is 2>selected</cfif> >Good</option>
        </select>
      </td>
    </tr>
    <tr>
    <td> Wt :
      ...
      <img src="/images/#edititem.cat#/#newitem#.jpg" height="200" > &nbsp; 
  ...
  <tr>
    <td colspan="3"> Stone1:
      ...
      <Select NAME="style">
        ...
        <option value="Silver Beaded" >Silver Beaded</option>
