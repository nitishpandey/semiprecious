...
<cfparam name="year" default="2010" />

<cfquery name="angelawholesale" datasource="gemssql">
SELECT Sum(buyingrecord_archive.cost*quantity) AS Cost, Sum(buyingrecord_archive.rate*quantity) AS Sales, Month([indate]) AS mnth, YEAR([indate]) as yr
FROM items, (select * from cartstatus_archive where indate >'1/1/07') as cartstatus INNER JOIN buyingrecord_archive ON cartstatus.cartid = buyingrecord_archive.cartid
where buyingrecord_archive.itemid=cast([items].[NEWITEM] as varchar(10)) AND cartstatus.paymode<>'null' AND (cartstatus.couponcode Like 'regwsale%' or cartstatus.couponcode Like 'wsale%')
...
SELECT Sum(buyingrecord_archive.cost*quantity) AS Cost, Sum(buyingrecord_archive.rate*quantity) AS Sales, Month([indate]) AS mnth
FROM items, (select * from cartstatus_archive where indate >'1/1/07') as cartstatus INNER JOIN buyingrecord_archive ON cartstatus.cartid = buyingrecord_archive.cartid
where buyingrecord_archive.itemid=cast([items].[NEWITEM] as varchar(10)) AND cartstatus.paymode<>'null' AND (cartstatus.couponcode not Like 'regwsale%' and cartstatus.couponcode not Like 'wsale%' or cartstatus.couponcode is null)
GROUP BY year([indate]),  Month([indate]), items.supplier
HAVING ((month([indate])=#month# AND YEAR([indate])=#YEAR# ) AND ((items.supplier)='Angela'))  order by mnth
</cfquery>
<!--- OC SALES --->
...
SELECT Sum(buyingrecord_archive.cost*quantity) AS Cost, Sum(buyingrecord_archive.rate*quantity) AS Sales, Month([indate]) AS mnth
FROM items, (select * from cartstatus_archive where indate >'1/1/07') as cartstatus INNER JOIN buyingrecord_archive ON cartstatus.cartid = buyingrecord_archive.cartid
where buyingrecord_archive.itemid=cast([items].[NEWITEM] as varchar(10)) AND cartstatus.paymode<>'null' 
GROUP BY year([indate]),  Month([indate]), items.supplier
...
SELECT Sum(buyingrecord_archive.cost*quantity) AS Cost, Sum(buyingrecord_archive.rate*quantity) AS Sales, Month([indate]) AS mnth, indate
FROM items, (select * from cartstatus where indate >'1/1/10' ) as cartstatus INNER JOIN buyingrecord ON cartstatus.cartid = buyingrecord_archive.cartid
where buyingrecord_archive.itemid=cast([items].[NEWITEM] as varchar(10)) AND cartstatus.paymode<>'null'  
GROUP BY year([indate]), Month([indate])
order by indate
</cfquery>
...
SELECT Sum(buyingrecord_archive.cost*quantity) AS Cost, Sum(buyingrecord_archive.rate*quantity) AS Sales, Month([indate]) AS mnth, indate
FROM items, (select * from cartstatus_archive where indate >'1/1/10') as cartstatus INNER JOIN buyingrecord ON cartstatus.cartid = buyingrecord_archive.cartid
where buyingrecord_archive.itemid=cast([items].[NEWITEM] as varchar(10)) AND cartstatus.paymode<>'null'  
