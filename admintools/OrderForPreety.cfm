...
<cfparam name="suppliername" default="preety">
...
<a href=../admintools/orderforpreety.cfm?orderby=cat>ORDER BY CATEGORY</a> | <a href=../admintools/orderforpreety.cfm?orderby=newitem>ORDER BY ITEM NUMBER</a>
...
Select  anup.restock.notes as notes, anup.restock.quantity as qty, newitem, 1 as monthstosell, disporder, weight, lastbought, basecost*0.7 as basecost, cat, '<img src=/images/' + cat + '/thumb/' + cast([newitem] as varchar) + '.jpg height=50>' as itemimage,
 '<A href=http://www.semipreciouswholesale.com/detail.cfm?newitem=' + cast([newitem] as varchar) +'>' +   cast([newitem] as varchar) + '</a>' as item, left([description],100) as description, internalnote, subcat from items inner join anup.restock on newitem=itemnum where orderfor like '%#suppliername#%'  order by   #orderby#
...
<cfif onlyitems is "yes">#newitem#<cfelse> #item#, #itemimage#  #subcat#,  #cat#, Wt:#weight# gms  #description#, #internalnote#, Star Rating:#monthsToSell#</cfif>
