...
<cfset session.mail = 'acemat@vsnl.com' />
</cfif>
<CFIF isdefined("session.passcode") and session.mail eq 'info@silverscorpio1.com' or session.mail eq 'acemat@vsnl.com'>
  <Cfoutput> 
  ...
  select * from cartstatus , buyingrecord, items where items.newitem= cast(buyingrecord.itemid as int) and buyingrecord.cartid=cartstatus.cartid and items.inventory <1 and supplier not in ('Angela', 'OC') and indate > #createodbcdate(dateadd('d',-url.no_of_days,createdate(datepart('yyyy',Now()),datepart('m',Now()),datepart('d',Now()))))#  or paymode in ('2co','auth.net','pp','chck','goog') order by 
cartstatus.cartid desc,   <cfif isdefined("url.orderby")> 
#url.orderby# desc
</cfif> 
</cfquery>
...
/sale)</td>
      ...
      <td>Out</td>
      <td>Coupon</td>
      <td><a href="cartstatus21.cfm?orderby=paymode" >Mode</a><br />
        </td>
...
<tr <cfif couponcode is 'regwsale' or couponcode is 'saved'> style="color:red"</cfif>>
        ...
        <td><a href="cartdelete.cfm?cid=#cartid#&&paymode=#paymode#&couponcode=#couponcode#" title="delete on next click">#decimalformat(inrcost)#</a></td>
