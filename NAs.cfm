...
Select  getdate() as lastbought, case 	when inventory>0 then '0' else '1'
   ...
   where storage='Angela' and inventory>0  <CFIF category NEQ "ALL">
      and CAT = '#category#' and cat <>'bags' </cfif><cfif subcat is not "">
...
<CFIF isdefined("session.bulkbuyer.id")>  AND (Storage ='NA' or Storage='pic room')</CFIF>
<!---
<cfif cgi.server_name IS "www.wholesale-gemstone-jewelry.com" or cgi.server_name IS "wholesale-gemstone-jewelry.com">
...
<CFOUTPUT>
	<TITLE style="text-transform:capitalize;">
		...
		#titleCase(color)# #titleCase(subcat)# #style#  #titlecase(p)# gemstone  
	</TITLE>
	
	<meta name="keywords" content="Affordable #color# #subcat#,Gem stone,  #p#">
	<meta name="author" content="Anup Pandey" />
	<meta name="description" content='#subcat# #style# #p# gem stone jewelry, retail and wholesale. <CFLOOP QUERY="Getlist" startrow="#start#"
			...
			<td width="612px" align="center" valign="top" style="border:1px ##DDDDDD ridge;">
				...
				<table align="center" cellspacing="0" border="0" cellpadding="3" style="margin-top:1px;background-color:##FFFFFF;">
					<cfif category is 'beads'>
						<tr><td colspan="<cfoutput>#displaycolumns#</cfoutput>" ><font color=green>beads being added daily Nov to December! Come back every day</font></td></tr>
					</cfif>
					...
					<tr class="greenbutton"  align="center">
						...
						<!---		<cfif url.sortorder neq "lastbought desc">--->
							<a href='gemstone_jewelry_gallery.cfm?category=#category#&sortorder=lastbought'>Recently Purchased by others</a> ||
						<!---		<cfelse>--->
							<a href='gemstone_jewelry_gallery.cfm?category=#category#&sortorder=pricedesc'>Premium First</a> || &nbsp;&nbsp;
						<!---	</cfif>--->
											
					<!---	<cfif url.sortorder neq "datetaken desc">--->
							<a href='gemstone_jewelry_gallery.cfm?category=#category#&sortorder=datetaken'>Latest Additions</a> || &nbsp;&nbsp;
							<!---<cfelse>--->
							<a href='gemstone_jewelry_gallery.cfm?category=#category#&sortorder=price'>Cheapest First</a> || 
							<!---</cfif>--->
&nbsp;&nbsp;&nbsp;				
...
<cfif cgi.server_name neq "www.ornamentscollect.com">  
          <font size=+3>NOW BEADS ON SEMIPRECIOUS.COM <a href='beads.cfm'>CLICK HERE</a><!---<cfinclude template="googleadsforlist.htm">--->
</cfif>
          </cfif>
             </td> </tr></cfif>
    </table>
