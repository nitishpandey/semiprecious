...
<CFIF cgi.server_name contains 'www.'><CFAPPLICATION name="wwwsemiprecious" setdomaincookies="no"   clientmanagement="yes"  sessionManagement="yes" setclientcookies="yes" clientstorage="cookie" sessiontimeout=#CreateTimeSpan(0,2,0,0)#/>
<cfelse>
...
--->	          <cfset light = 'pink' />
  ...
  <cfset application.tollfree = '<font color="purple">512-666-GEMS(4367)</font>' />
	<CFIF (not isDefined("session.mail") ) or (not isdefined("session.cartid")) or (not isdefined("session.cartitem"))><!--- => session start. make sure session.start is not initiali---->
                <cftry>
		...
		<cfcatch type="any">
                ...
                <cfdump var="#cfcatch#" /><cfif not isdefined("session.mail")>
...
<!---	<CFAPPLICATION name="semiprecious"  sessionTimeOut="#CreateTimeSpan(0,0,0,0)#" sessionManagement="yes" />--->
		   <cfdump var='#cfcatch#' />
			There was an internal error or our browser cookies were corrupted or the server just crashed on you. Click <a href="/logout.cfm">here</a> to continue.
