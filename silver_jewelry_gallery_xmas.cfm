...
<cfquery datasource="gemssql" name="getlist2" >
	select * from items where newitem = #getlist.newitem# 
</cfquery>
<cfelse>
<cfquery datasource="gemssql" name="getlist2" >
	select * from items where newitem = 9042 
</cfquery>
...
<cfmail to="anup@semiprecious.com" server="mail23" subject="nothing in getrecords...gallery reporting" from="service@semiprecious.com"
 type="html" >
 #cgi.HTTP_REFERER# 
 #subcat#,#category#,#url.sortorder#,#session.mail#,#advancedsearch#,#salestatus#,#color#,
#advancedsearch1#,#advancedsearch2#
 #cgi.QUERY_STRING#  #cfcatch.Detail# #cfcatch.Message# #cfcatch.Type#
</cfmail>
...
<cfoutput>
  ...
  <CFLOOP QUERY="Getlist" startrow="#start#" endrow="#evaluate(start+1)#">#xmlformat(getlist2.description)#. 
		...
		<div style="float:left;display:inline;width:143px;">
  			<cfset leftgemstonedisplay="inline" />
				<cfif color neq "" >
					...
					<cfif category neq 'gems' and category neq 'brooches'>
					 <CFINCLUDE TEMPLATE="leftmenus.cfm"></cfif>
				</div> 

 <div id="semiprecious_gallery" style="margin-top:1px;overflow:visible;position:relative;float:right;margin-right:2px;" >
<CFIF getList.recordcount is 0>
     <div style="margin-top:10px;margin-left:50px;border : 1px solid #ECCCC;	width:607px;text-align:justify;font-size:12px;color:maroon;">
     	<cfoutput>
     		 Sorry no matches found for your criteria &rarr;#category# #subcat# #advancedsearch# #color# #format(priceless)# to #format(pricegreater)#.  <br />Please try with some other selectors given in left panel and top panel. 
    	</cfoutput>
    <cfset silver = "">
...
<br /><img src="images/filter.jpg" style="display:inline" width="16px" /> Filter: <cfinclude template="/includes/searchdropdowns.cfm" />
  ...
  <div id="infotable"  style="margin-left:20px;"  align="justify" > 
