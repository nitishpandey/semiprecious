...
{
	document.getElementById('custom_state').innerHTML = response;
	return;
}
function custom_select(i,s)
{
var url_ = '/customrequest/custom.cfc?method=custom_status&itemid='+i+'category='+cat;
loadXMLDoc(url_);
...
return;}
	...
	<cfif details.recordcount  and category neq ''>
...
<div align="center" >
<div style="width:930px;background-color:white;">
  ...
  <div style="border:2px #DADA99 ridge; position:relative;margin-top:2px;padding-right:1px;background-color:white;width:100px" > 
    ...
    <a href="/showearrings.cfm" >earrings</a> <br />
...
<cfelse>
	<!--- No category --->
	<!--- End no category --->
	<div style="float:left;width:145px;"><cfset leftgemstonedisplay="inline" />
	...
	<div style="width:780px;float:right;">
        <CFSET folder = LCASE(category) />
          ...
          <cfoutput>
            ...
            </cfoutput><div style="float:left;width:560px">
              ...
              <tr valign="top" >
                ...
                <table border="0" cellpadding="0" cellspacing="0" style="padding:1px" >
					...
					&nbsp;|&nbsp;<span style="color:black;">Item Id: #newitem#</span>&nbsp;|&nbsp;	<span   style="positio:absolute;top:8px;left:2px;z-index:1;background-color:transparent;" > 
						...
						<div   class="form_heading" style="color:black;">
				...
				<span class="greenbutton" style="border-bottom: purple 2px outset; border-left: purple 2px outset; background-colo: whit; width: 200px; height: 28px; border-top: black 2px outset;  font-weight: 800; border-right: purple 2px outset; padding-top: 4px; left: 2px; positio: absolute;"
				onCli="document.getElementById('cart_form').innerHTML=document.getElementById('easy_form').innerHTML;"  onClik="goProcess('cart summary');">&nbsp;<a title="Check Cart and start checkout" href="/#session.cart#?country=#urlencodedformat(session.address.country_code)#&amp;php=1.2210&amp;cartid=#session.cartid#" >Start Checkout &rarr;</a>&nbsp;
						  </span>
						  <cfelse>&nbsp;
                   ...
                   <!---  <br style="clear:both" >--->
                    ...
                    <cfif details.morepics GT 0 and (not len(width))>
                      <div style="border: 1px gray ridge;padding-top:4px;background-color:#EFEFEF" align="center" > 
                        <cfoutput> 
                        <img  src ='/images/#folder#/<CFIF #NameID# is "" >#newitem#.jpg<CFELSE>#NameID#.jpg</CFIF>'   height="70px" border="1" style="cursor:pointer" onMouseOver="swapMainImage('/images/#folder#/#imagename#');"  alt='unique #subcategory# #category# Jewelry' > 
...
This is out of stock, request us to make for you                       
                        </cfif>
                        #description#<br />
                        <b>Size:</b> 
...
Ct.
	             ...
	             <p> <a href='/<cfoutput>#stonename#</cfoutput>.htm'>
				   <font size=+1>Learn about this stone</font></a>
            </cfif>
...
<!---    	union
	...
	--->
		...
		<cfif inventory lt 1><br /><i>sold out</i></cfif></td>
		<cfif coll is 5></tr>
        <tr>
		<cfset coll=0>
        </cfif>
        ...
        <cfloop from="1" to="#missingtds#" index=k><td></td></cfloop>
        </tr>
        </table></div>	
        </cfif>
                  <br />
          ...
          <br />
                  <cfif session.mail is application.admin_email>
                    ...
                    <cfif category is 'bags'>
                      <a href="/editbags.cfm?newitem=#newitem#"  >EDIT
                      </a>
                      <cfelse>
                      <a href="/admintools/edititem.cfm?newitem=#newitem#"  >EDIT</a>
                    </cfif>
                    
                    <cfquery name="custom_state" datasource="sptm">
                    select mask , status from custom_items where imageid = '#newitem#' 
                    </cfquery>
                    Current State:
							<span id="custom_state">
                                        <cfif custom_state.recordcount>
			                            <cfoutput>#custom_state.mask# + #custom_state.status#)</cfoutput>
            							<cfelse>
										Not Selected
                                        </cfif>
                                        </span>                                        
                    <form action="none_required_ajax">
<cfoutput>Activate<input name="custom_select" type="radio" onClick="javascript:custom_select(#newitem#,'select');" value="select">
Mask <input name="custom_select" type="radio" onClick="javascript:custom_select(#newitem#,'unselect');" value="select">
          </cfoutput>     </form>
                  </cfif>
                    ...
                    <CFIF price eq 0 and (len(session.bulkbuyer.id) is 0 )>
                      ...
                      <cfif status eq 0 or status eq 3 >
                        ...
                        <cfif details.basecost >
							...
							#round_format(8.18*details.basecost/application.exchangerate)# </span></cfif>
							<cfif len(session.bulkbuyer.id)>
							<br />Wholesale:
                          <cfelse>
						 ...
						 --->    <cfif optcount and not (url.optionid)>
	                        	 <cfquery datasource="gemssql" name="opts" >
                            	  ...
                            	  <cfif opts.recordcount >    
							  		 <div  class="optionsinline" align="right" style="float:right;width:100%;horizontal-align:right" >
							...
							<input style="width:101px;height:22px;margin-top:2px;;font-weight:800;" class="green_bg" type=submit value="Add to Cart" name="Add to cart">

    <!---                        <img src=/images/addtocart.gif d="a2cb"  onClick="javascript:goProcess('add')" >   --->
							<input type="button" class="hidden" id="a2cw" value="Wait..." >
                          </cfif>
                          <!--- Angela --->
						 <cfif not len(width)> <INPUT TYPE="button" id="a2wl" style="width:100px;margin-top:14px;border:1px purple solid;" class="greenbutton" onClick="javascript:goProcess('Add to Wish List')" VALUE="Add To Wish List" >
...
--->        
		  ...
		  <cfif 1> <!--- if width variable set by cfinvoke add is set to thumb then next column --->
				...
				Select top 6 newitem, cat from items where subcat='#subcategory#' and cat <>'#details.cat#' and cat <> 'beads' and inventory>0 order by newitem desc
				</cfquery>
