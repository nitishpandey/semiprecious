...
<cfset session.bulkbuyer.minamt=20> <!--- special email sent minimum reduced for wholesale --->
...
font-family:arial;
font-size:1.4em;">
<cfif true> <cfoutput><img src="/images/moreshopping.gif" style="margin-left:10px;border:2px gray ridge;left:10px;margin-top:3px;cursor:pointer;width:90px;display:inline;" onClick="javascript:goProcess('buy more')" ></cfoutput> 
...
<span style="margin-left:80px;">REVIEW FOR YOUR ORDER (ORDER ID:</span> <font color="black"><cfoutput>#session.cartid#</cfoutput></font>)</div>
<!--- <cfif len(cart_msg)><div align="center" class="green_band_ticked"><cfoutput>#cart_msg#</cfoutput></div></cfif>
 ---></div><p>&nbsp;
<p>
<TABLE bordercolor="#FFF" style="display:table;margin-left:6px;border:1px #fff solid;width:772px;" cellspacing="0" align="center"  >
...
<TH width="18%" >	<cfif checkout is 'no'><img src="/images/que.gif" style="display:inline;" onClick="MM_showHideLayers('look_set_hint','','show')" />We Suggest</cfif>  </TH>
  <TH width="15%" colspan="1">	<cfif checkout is 'no'>Edit Cart</cfif></TH  >
  ...
  <TH width="12%">Amt (#session.currency#)</TH>
    ...
    <cfset rowb = "rowb" & (j mod 2)/>
          <CFQUERY NAME="qprice" DATASOURCE="gemssql" timeout="400"    >
           SELECT
                <cfif session.cartitem[j][5]> options.description as optsdesc, </cfif>  storage, subcat+ ' ' + cat as description,clustercount,status,storage,thumblink,orderonrequest,items.inventory  FROM Items with (nolock) 
    <cfif session.cartitem[j][5]> inner join options with (nolock) on items.newitem=options.itemid  </cfif>
    WHERE   items.newitem = #session.cartitem[j][1]# 
    <cfif session.cartitem[j][5]> and options.optionid = #session.cartitem[j][5]#</cfif>
    </CFQUERY>
	     <cfoutput>
		 <tr class="#rowb#" id="#session.cartitem[j][1]##session.cartitem[j][5]#" >
												...
												<cfcase value="2">
														...
														<cfcase value="3">
																		<span class="lookinline">
																			<a href="gemstone.cfm?itemid=#session.cartitem[j][1]#&type=look">&nbsp;Similar&nbsp;</a>
																		</span><hr />

																		<span class="setinline">
																			<a href="gemstone.cfm?itemid=#session.cartitem[j][1]#&type=set" >&nbsp;Set&nbsp;</a>
																		</span>
														</cfcase>
													<cfdefaultcase >&nbsp;
													</cfdefaultcase>
...
</font><img src="/images/add_item.jpg" width="12" height="12" alt="buy more"> <!---
...
<a href="javascript:reduce_item(#session.cartitem[j][1]#,#session.cartitem[j][5]#,0,#session.cartitem[j][3]#);"><font color="##440000">Remove</font></a> <img src="/images/trash.jpg" width="12" height="12" alt="delete">
   
   </cfif>
</td>
            <td align="left">
						<cfif checkout is 'no'><img src="/images/#session.cartitem[j][2]#/thumb/#session.cartitem[j][1]#.jpg" border="0" width="80"></cfif></td>
            ...
            <td   align="center">  <span id="added_#session.cartitem[j][1]##session.cartitem[j][5]#">#session.cartitem[j][4]#</span>
            </td>
            
            <td  ><span id="amount_#session.cartitem[j][1]##session.cartitem[j][5]#">#decimalformat(value_convert(amounts))#</span>
              <input type="hidden" name="amount#j#" id="amount#session.cartitem[j][1]##session.cartitem[j][5]#" Value="#amounts#">
            </td>
			</tr>
<cfif qprice.inventory LT 1 and qprice.orderonrequest is 1><td colspan=6 align=center bgcolor=red><font color=white> THE	ABOVE ITEM IS CUSTOM MADE FOR YOU, THIS WILL DELIVER IN 3 WEEKS, REST WILL SHIP IN 24 hrs</font>
<cfelse><td colspan=6 align=center><font color=purple>ABOVE ITEM SHIPS IN 24 to 48 hrs</font>
</cfif>
</td></tr>
          </cfoutput> 
...
Shipping Discount for Orders over #format(application.retail_free_shipping)# [Free within US]<br />
 ...
 <tr><td colspan="2" align="center"><a href="https://<cfoutput>#cgi.server_name#</cfoutput>/cart.cfm?country=<cfoutput>#session.address.country_code#</cfoutput>&checkout=yes">
   ...
   <img src="/images/moreshopping.gif" style="border:2px gray ridge;width:120px;cursor:pointer" onClick="javascript:goProcess('buy more')" > &nbsp;
		...
		<tr ><td colspan=2 align="center">	 <tr ><td colspan=2 align="center">   <img src="/images/moreshopping.gif" style="border:2px gray ridge;width:120px;cursor:pointer" onClick="javascript:goProcess('buy more')" > &nbsp;
				...
				<tr><td colspan=2 bgcolor=##948 style="color:white;" >
					<cfif len(session.bulkbuyer.id)>Volume discount shown below (for carts over #format(500)#/#format(1000)#/#format(2000)#) will be applied on the next stage of checkout
					</cfif>&nbsp;</td>
              </TR>			
	   		<cfif len(session.bulkbuyer.id)>
		 <tr >
          ...
          <tr>
				...
				<select name="SpecialHandling"  >
				  ...
				  <cfif expedited_possible  >
					   <cfif url.country  is "USA" or url.country  is "AU" or url.country  is "GB" or url.country  is "NZ">
						    		<cfset hand_msg ="[Standard: Ships in 24 hrs]" />	
						  			<cfif url.country  is "USA" or url.country  is "AU">
												<option  value="1.1">Priority (2-3 Days Delivery in USA) Add $1.10</option>
				
						  			</cfif>
											<option   value="14.99">Express (1-2 Days Delivery in USA) Add $14.99</option>
							<cfelse>
										<cfset hand_msg ="[Shipped in 24 hrs from Austin]" />	
										<option   value="14">USPS Express Add $14.00</option>
						</cfif>
					  	<cfelse>
              ...
              <cfoutput>
               ...
               <input type=hidden name="zip" value="#session.address.zip#" size="15">
				...
				<tr><td colspan="2" bgcolor="##603082" >&nbsp;</td></tr>
				  ...
				  <tr>
                  <td  align="left"><font size=+1><p>Ship to Country</font>:</td>
                  <td><cfinclude template="/includes/countries_drop_down.htm" />&nbsp;</td>
                </tr>
			  ...
			  <cfelse>
			  <tr>
           	    <td align="left"><font size=+1>Email</font><cfif 1 or url.country eq "USA">(<span class="form_heading">Optional</span>)</cfif>: </td>
              ...
              <tr>
                <td  align="left"> <font size=+1>Shipping Options</font>: <br />
				 </td>
                <td  align="left">
							...
							<select name="SpecialHandling" >  
									<cfif expedited_possible  >
										...
										<option   value="2.00"> Priority Mail (1-2 Day Delivery) $<cfoutput>#evaluate(shipping+2)#</cfoutput></option> 
									 	<option   value="10.00"> Express (1-2 Day Delivery) $<cfoutput>#evaluate(shipping+10)#</cfoutput></option> 
									<cfelse>
										<cfset hand_msg ="[Standard: Ships in 24 hrs]" />	
									<option   value="0"> Standard (USPS with tracking )</option>
									
									<option   value="2.00">Priority Mail (1-2 Day Delivery) $<cfoutput>#evaluate(shipping+2)#</cfoutput></option> 
									 	<option   value="14.99">Express (1-2 Day Delivery) $<cfoutput>#evaluate(shipping+14.99)#</cfoutput></option> 
									</cfif>
							  </select><br />
					  	...
					  	<cfelse>
								    ...
								    <select name="SpecialHandling" >
									      			<cfset hand_msg ="[Standard: Ships in 24 hrs]" />	
											<option    value="0.00" selected >Standard Ships in 24 hrs</option>
								  <option   value="2.00">Priority Mail (1-2 Day Delivery) $<cfoutput>#evaluate(shipping+2)#</cfoutput></option> 
									 	<option   value="14.99">Express (1-2 Day Delivery) $<cfoutput>#evaluate(shipping+14.99)#</cfoutput></option> 
...
--->
		   <span style="color:gray;">Coupon or Gift Code (If Any):</td><td> <input name="couponcode" type="text" class="detail" value="" size="12" maxlength="18">   &nbsp;&nbsp;&nbsp; Agent Code, if any: <input name="agentcode" type="text" class="detail" value="" size="12" maxlength="8"><br />	    
			      ...
			      <tr>
	                <td  align='left' nowrap><img src="/images/gift.gif" align="absmiddle" vspace="0" hspace="0" border="0"   height="22" style="display:inline;" />&nbsp;&nbsp;<font color="#000000" size=+1>Gift Wrap</font>: </td>
	              ...
	              <tr id="giftmsg">
	                <td align="left">  <font size=+1>Message (If Gift)</font> <span id="j"  style="text-decoration:underline" ><a onClick="MM_showHideLayers('giftmessage','','show')" style="cursor:pointer">[?]</a></span>:</td>
			  ...
			  <cfif len(session.mail) and session.loyalty_points and session.mail contains "@">
		                <tr>
		                  <td align="left">  <font size=+1>Loyalty Discount</font><a href="viewinvitecredits.cfm" ALT="friends invited for jewelry site">[?]</a>:</td>
