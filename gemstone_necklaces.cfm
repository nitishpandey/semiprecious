...
Select disporder, newitem, m.CAT, subcat, datetaken,  NameID,     price  , saleprice, status, wholesaleprice, Description, inventory, storage, thumblink, imagelink,
   buylink from items m, ltbstones s where s.cat = m.cat and s.stone=m.subcat and 
newitem =(select top 1 newitem from items where cat=m.cat and subcat=m.subcat and inventory>0) and (STATUS = 0 OR STATUS=3) <CFIF CATEGORY EQ "ALL"> AND M.CAT <> 'BEADS' AND M.CAT <> 'HEALING'</CFIF> <cfif category neq "all">and m.cat='#category#'</cfif> 
<CFIF isdefined("session.bulkbuyer.id")> AND (Storage <> 'pippin') and (storage not like 'CM%')</CFIF>
<cfif cgi.server_name IS "www.wholesale-gemstone-jewelry.com" or cgi.server_name IS "wholesale-gemstone-jewelry.com">
...
<cfif getlist.storage EQ 'Pippin'>
 <a href=gem_stone_#cat#.cfm/#item_array[columns]#.htm> 
              <img src ="#thumblink#" longdesc="http://www.semiprecious.com/longdesc.cfm?newitem=#newitem#" border='0' bordercolor='black'  name="img#newitem#" title="#subcat# #category#" <cfif category is 'bracelets'>width="100"</cfif>></a>
<CFELSE>
 <a href='gem_stone_#cat#.cfm/#item_array[columns]#.htm' > 
...
<a href="np/match/cluster.cfm?list=1&target=#newitem#&category=#cat#">Ad2Clus</a>
			...
			<cfswitch expression="#status_array[column]#">
                  ...
                  <b>Zoom</b></a>&nbsp;&nbsp;&nbsp;#price_array[column]#</font>&nbsp; <br />&nbsp;<font color="##FF4444"><cfif cat is 'beads'>Launching Nov 22!<cfelse>Sold Out</cfif></font>
...
<br /><a href='gem_stone_#cat#.cfm/#item_array[column]#.htm' ><font color="##CC3300">Buy</font></a></cfdefaultcase>
				  </cfswitch><div  align="justify"  class="listdescr">#left(description, 100)#... 
<cfif not isdefined("session.bulkbuyer.id")> Wholesale <a href='wholesale_jewelry.cfm'>here</a></cfif>
 </div> </td>	
