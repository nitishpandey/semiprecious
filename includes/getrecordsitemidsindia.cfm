...
<cfoutput>
  Select top 1200 status, newitem, price, case 	when inventory>0 then 1 else 0
		end as instock   from itemsindia 
<!---  <cfif subcat is "" and color is ""> inner join (Select MAX(itemid) as itemid from  styles group by styleid) style on newitem=itemid	</cfif>--->
   ...
   where (inventory>0 	<cfif cgi.server_name does not contain 'wholesale'>or orderonrequest=1</cfif>)  
