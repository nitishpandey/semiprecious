	...
	<cfif sortorder contains 'lastbought'>
		<cfset sortorder='totalqtysold desc'>
	</cfif>
...
--->
	<cfif sortorder is "datetaken desc">
		<cfset sortorder="newitem desc" />
	</cfif>

<cfoutput>

 <cfif (sortorder is "totalqtysold desc" or sortorder is "newitem desc") and
  priceless eq "" and groupname eq "" and
  color eq "" and subcat is "" and style is "" and advancedsearch is "">
		Select top 2000 status, newitem, orderonrequest, price,  cat, color, subcat,grouping,size,weight, case 	when inventory>0 then '0' else '1'
		end as instock      <cfif (sortorder contains "totalqtysold")>, totalqtysold from itemspopular<cfelse>
   from items  </cfif>

     where <cfif session.country is 'india'>storage like 'IN%' and </cfif> storage='Angela' and inventory>-1 <CFIF category NEQ "ALL">
  ...
  Select top 2000 status, newitem,  orderonrequest, price, cat, color, subcat, grouping,size, weight,case 	when inventory>0 then '0' else '1'
    ...
    <cfif (sortorder contains "totalqtysold")>, totalqtysold from itemspopular<cfelse>
   from items  </cfif>
   <cfif groupname neq ""> inner join  (select itemid from itemsbyGroup  where groupname='#groupname#') gvr on newitem=itemid </cfif> 
   where <cfif session.country is 'india'>storage like 'IN%' and </cfif> (inventory>0 or orderonrequest=1)  <cfif subcat neq "cubic zirconia"> and (storage not like 'CM%')</cfif>
<CFIF cgi.server_name contains "wholesale" or session.bulkbuyer.id neq ""> AND WHOLESALEPRICE >0 and (storage not like 'CM%')
<CFELSE>
and price>0 
</CFIF>
and cat <>'ornaments'
         <CFIF category NEQ "ALL" and category neq "">
           and (CAT = '#category#'  <!---or CAT2 = '#category#'--->
     	    ...
     	    <cfelse>
           and 
		(CAT <> 'beads' and cat <> 'bags'  and cat <>'healing' and cat <> 'gems' and cat <>'chains'   )
     	   </CFIF>
   <CFIF salestatus is 3>
     and (status = 3)
    <CFELSEIF salestatus is 1>
  ...
  <CFIF isnumeric(priceless)>
    ...
    <cfset ul = priceless >
    <cfset sortorder = sortorder & ', price desc' />
...
subcat like '%#advancedsearch1#%' <cfif advancedsearch2 is not "">or subcat like '%#advancedsearch2#%'</cfif> <cfif advancedsearch3 is not "">or subcat like '%#advancedsearch3#%'</cfif>
  ...
  <cfif ringsize neq ""> and newitem in (select  itemid from options, items where cat='rings' and options.itemid=items.newitem  and options.description='#ringsize#' )  </cfif>
    ...
    order by  instock,   #sortorder#, status    
