...
<cfoutput>
 <cfif (url.sortorder is "lastbought desc" or url.sortorder is "newitem desc") and
  priceless eq "" and groupname eq "" and
  color eq "" and subcat is "" and style is "" and advancedsearch is "">
Select itemnumber as optcount, grouping, color, getdate() as lastbought, case 	when inventory>0 then '0' else '1'
   ...
   buylink ,  orderprice from (SELECT top 4 0 as orderbuy, itemnumber,  grouping, color, lastbought, restockdate, disporder, disporderwhole, newitem, CAT, style, subcat, subcat2, datetaken,   NameID,     price  , saleprice, status, wholesaleprice, Description, inventory, storage, thumblink, imagelink,
   buylink , case status when 3 then saleprice else price end as orderprice from items  
     inner join (Select top 100 sum(quantity) as qty, itemid as lbitem from buyingrecord where  datebought >'8/1/2009'  group by itemid order by qty desc
) lb on lb.lbitem = items.newitem where storage='Angela' and inventory>-1  <CFIF category NEQ "ALL">
  ...
  Select itemnumber as optcount, grouping, color, case 
