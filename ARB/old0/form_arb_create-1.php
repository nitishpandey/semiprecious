	...
	<td colspan="2"><input disabled value="<?php echo "$".round(($amount/3), 2); ?>">
			...
			<div>Your card will be charged as per the following Billing Schedule:<br>
				 <b><?php echo "$".round(($amount/3), 2); ?>: Today </b><br>
				    ...
				    <?php echo "$".round(($amount/3), 2); ?>: <?php echo date("F j, Y", mktime(0,0,0,date("m")+2,date("d"),date("Y"))); ?>
...
document.getElementById('billingDiv').style.display = 'none';
        ...
        <div id="billingDiv" style="float:right; display:none"> State <i>(if outside US)</i>:<input type ="text" name="billing_state_ous" maxLength="14" size="14"> <!-- disabled="disabled"> -->
        </div>
      </td>
...
<option value="" selected>Choose a Country</option>
    ...
    <tr align="left" valign="top" bgcolor="f0f0f0"> 
      <td class="field_name">
      <vllabel validators="phoneNumber_required,phoneNumber_syntax" errclass="error" for="phoneNumber" cerrclass="controlerror">Phone number:</vllabel>
      </td>
<td><input type="text" name="phoneNumber" maxLength="25" size="25" ></td>
...
document.getElementById('shippingDiv').style.display = 'none';
        ...
        <div id="shippingDiv" style="float:right; display:none"> State <i>(if outside US)</i>:<input type ="text" name="shipping_state_ous" maxLength="14" size="14">
        </div>
        </td>
...
<td><input type="text" name="cardNumber" maxLength="16" size="20" id="cardNumber" lang="Credit Card Number Required" value="xxxxxxxxxxxxxxxx"></td>
  ...
  <vlgroup operator="OR" errmsg="Can't have a US Billing State when country selected is not United States">
 ...
 <!-- CHECK IF *SHIPPING* STATE IS NOT SELECTED AS OUTSIDE-US WHILE COUNTRY IS ALSO SELECTED AS U.S -->
   <vlgroup operator="OR" errmsg="Can't have a US Shipping State when country selected is not United States">
