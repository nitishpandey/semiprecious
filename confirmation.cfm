...
<cfscript>
	if (isdefined('form.stripeToken')) {
	    //&& StructKeyExists(form, 'fieldnames') && len(form.fieldnames)
		// initialize gateway
		cfg = {
		path = "stripe.stripe", TestSecretKey = "sk_test_4pgLGlAau7eiMw1FtwoXurEm"
	};
	...
	// charge the one-time token from stripejs by converting it to a cfpayment token and passing to purchase as the account //response = gw.purchase(money = amount, account = token);
	// charge the customer instead by converting the customer token (cus_*) into a cfpayment token and passing to purchase customer = gw.store(token);
	writeDump(customer.getTransactionId());
	customer_token = svc.createToken().setID(customer.getTransactionId());
	response = gw.purchase(money = amount, options = {
...
<cfoutput>
     ...
     <table cellpadding="0" cellspacing="0" ><tr><td align="left"><p>Order Summary<br />
		...
		Keep Track Of Your Order:
					<a href="https://www.facebook.com/sharer.php?u=http%3A%2F%2Fwww.#session.tld#/itemsell.cfm?cartid=#form.x_invoice_num#"  style="border:none; overflow:hidden; width:90%; max-width:350px; height:35px;" >Place On My Facebook Account</a>
					</td>
					</tr>
					</table>
<br>
...
#err_mess#,<cfdump var='#form#' />
	...
	<script>
		try {
			$(document).ready(function() {
				...
				$('#sameas').click(
				function() {
					if ($('input#sameas:checked').val() == 'on') {} else {}
				});
			});
		}
		catch (err) {}
		...
		<div id="container2">
			<div class="form_heading" style="margin-top:-30px;text-align:left;<cfif not mobile>width:50%;</cfif>font-weight:bold;padding-left:2px">
				Check Your Final Total and Select A Secure Payment Method
			</div>
			<cfset timestamp = Now() />
			<cfset couponcode = trim(couponcode) />
			...
			<cfelse>
				...
				<table>
					<tr>
						<td bgcolor="#FFFFFF" valign=top align="right" style="max-width:440px">
		...
		</div> <cfinclude template="mainfooter.cfm" /> <cfabort /> </cfif>
		...
		<table  id="container3" style="width:400px;margin-top:20px;border:1px gray solid;" border="0" align="center"  >
			<cftry >
						...
						<cfoutput query="qprice">
							...
							<cfif  (qprice.inventory LTE 0) and qprice.orderonrequest >
								...
								<cfset aodtext ='
									...
									This design will be custom made against your order. This will take about 12 days and your cart will ship in about 3 weeks from Austin.</span>' />
							</cfif>
							<!---	<td>
								www.semiprecious.com/shaped/heart-gemstone-jewellery.cfm?Error Executing Database Query.,[Macromedia][SQLServer JDBC Driver][SQLServer]Invalid column name 'country'
						...
						#giftmsg# for cart #session.cartid#
							...
							<tr>
								...
								<td align="right">
									...
									<!--- value being added and shown as shipping #format(GiftWrap)# --->
								</td>
							</tr>
						<cfelse>
							<cfset gw = "">
  ...
  password="Prec1ou5sx@"  from="service@semiprecious.com"  subject="At confirmation Could not reverse "  type="html">
				...
				<cfif isdefined("wholefault")>
					...
					<tr >
						<td colspan="2" align="center" valign="middle" style="padding:2px;background-color:#FFE4EC;border:2px solid #A50A25;color:#A50A25;font-size:2em;" >
					...
					<cfoutput>
						<cfif ((coupon_saving and (not loyalty_discount) and couponflag)  and isdefined("tpsale") ) or len(session.bulkbuyer.id) >
							<tr class="" >
								...
								<td align="right">
									#format(evaluate(coupon_saving+amount-shipping))#
								</td>
							</tr>
							<cfif coupon_saving>
								<tr class="" >
									<td  align="right">
										#coupon_msg#. Your Total Savings/Discount:
									</td>
									<td align="center" class="error1" style="text-align:right;">
										#format(coupon_saving)#
									</td>
								</tr>
							</cfif>
                	...
                	update buyingrecord  set rate = #bbdf#*rate where cartid = #session.cartid#
						...
						<cfelseif gift_value LT 0 and cgi.server_name does not contain 'laptop'>
							<tr class="reg_price" >
								...
								<td align="center" class="error2" style="text-align:right;">
									&nbsp;
								</td>
							</tr>
						</cfif>
						<cfif cgi.server_name does not contain 'wholesale'  and (session.x_ship_to_state eq 'TX' or session.x_ship_to_state eq 'texas')>
							<tr>
								<td  align="right" >
									<cfset salestax = (amount-shipping)*0.0825 />
									<cfset amount=salestax+amount>
									Sales Tax:
								</td>
								<td align="right" >
						...
						<tr>
							<td align="right" style="padding:1px;border:1px 0 1px 0 solid gray">
								Shipping &amp; Handling:
							</td>
							<td align="right" style="padding:1px;border:1px 0 1px 0 solid gray">
								#format(shipping)#
  ...
  password="Prec1ou5sx@"  from="service@semiprecious.com"  subject="Error at confirmation">
			...
			<tr>
				<td align="center" style="background-color:#DDD">
					<div >
						<font size="" color="#000099">
							<a name="grand_total" style="text-decoration:none;">
								<strong>
									Pay Below
								</strong>
							</a>
						...
						or
						<cfoutput>
							<a href="#session.filter#&em=#confirm_mail#&dest=shop" >
								Continue Shopping
							</a>
						</cfoutput>
						&nbsp;
			...
			<tr>
				<td  height="20"  align="center" class="greenbutton">
					<img src="/images/secure_lock_2.gif" width="16" height="17" style="display:inline;" border="0" vspace="0" hspace="0" align="absmiddle">
					<b>
						Most Secure &amp; Best CREDIT CARD PAYMENT SYSTEM
					</b>
				</td>
			</tr>
			<tr>
				...
				<td align=left valign=top colspan="2">
					<cfif amount LT 0.1 and couponcode neq "">
						<div id="noneedtopay" style="position:absolute;height:200px;font-color:#993333 ;font-weight:bold;font-size:16;font-face:arial">
							YOUR ORDER IS COMPLETE, NOTHING FURTHER TO DO. THANK YOU.
						</div>
						<p>
						<p>
					...
					<cfif amount LT 1 and amount GT 0.1 and couponcode neq ""> <div id="noneedtopay" style="position:absolute;height:200px;font-color:#993333 ;font-weight:bold;font-size:16;font-face:arial"> YOUR ORDER IS COMPLETE, NOTHING FURTHER TO DO, WE WILL TAKE ARE OF THE TINY AMOUNT LEFT IN THIS ORDER. THANK YOU</div><p><p> <cfmail to="semiprecioushelp@gmail.com"  server="MailA40" subject="Order Redeemed by Coupon" from="cs@semiprecious.com">
								 FYI: Order <cfoutput>#session.cartid#</cfoutput> has redeemed a coupon, not needing payment by CC
			     			 </cfmail> </cfif>
<p><p> <h3><a style="text-decoration:none;" name="payment"><b>Please Pay Using </b></a><cfoutput>
		 <form name="_xclick" action="https://www.paypal.com/cgi-bin/webscr" method="post"  style="display:inline;"> <input type="hidden" name="cmd" value="_xclick"> <input type="hidden" name="business" value="anup@semiprecious.com"> <input type="hidden" name="currency_code" value="USD"> <input type="hidden" id="itemname" name="item_name" value='#session.cartid##sh##Gw#' /> <input type="hidden" name="invoice" value='#session.cartid#' /> <input type="hidden" name="return" value="http://www.semiprecious.com/paypalreturnurl.cfm" id="PayPalReturn" /> <input type="hidden" name="rm" value="2" id="ReturnMethod" /> <input type="hidden" name="notify_url" value="http://www.semiprecious.com/paypalreturn.cfm?xcartid=#session.cartid#" id="PayPalNotifyUrl" /> <input type="hidden" id="amount" name="amount" value='#decimalformat(amount)#'> <input width="70px" type="image" src="https://www.semiprecious.com/images/paypal_logo.gif" border="0" name="submit" alt="Make payments with PayPal - it's fast, free and secure!"> <!--- Newly added variables--Start ---> <input type="hidden" id="cartinvoicenumber" name="custom" value="#session.cartid#">
		  <!--- Newly added variables--End ---> </form> </cfoutput>
		<cfset session.amount= decimalformat(amount) /> </h3> <!--- still in sandbox mode

			<script type='text/javascript' src="https://static-na.payments-amazon.com/OffAmazonPayments/us/sandbox/js/Widgets.js"></script>
			<h3>or pay using your Amazon account</h3><div id="AmazonPayButton"></div>

			<script type="text/javascript">
			OffAmazonPayments.Button("AmazonPayButton", "AZYH4AWC85UB9", {
			type: "hostedPayment",
			hostedParametersProvider: function(done) {
			$.getJSON("ExpressSignature.php", {
			amount: parseInt($("#amount").attr("value")),
			currencyCode: 'USD',
			sellerNote: $("#itemname").text() + ' Cart/Invoice No: ' + $("#cartinvoicenumber").val(),
			//csrf:$("#csrf").val()
			}, function(data) {
			done(data);
			})
			...
			---> <!---As a security feature authorize.net button is <span style="color:red">valid for next 3 minutes</span> only.---> <!---
			...
			---> </td> </tr>
				...
				<td align="left" >
					...
					<cfoutput>
						...
						<a href="http://#cgi.SERVER_NAME#/myorder" style="text-decoration:underline">
							My Orders
						</a>
					</cfoutput>
				</td>
		...
		<script language="javascript1.1" >
			...
			var url = '/cartcontrol/cartmonitor.cfc?method=testconfirm&cartid=#session.confirmed_cartid#'; < cfcatch type = "any" > < /cfcatch></cftry > < /cfoutput>
