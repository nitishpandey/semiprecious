	...
	<CFQUERY DATASOURCE="gemssql" NAME="stats" >
	  ...
	  <cfif session.bulkbuyer.id neq ""  >
		  ...
		  <cfif findnocase('silver',details.style)>
		    <cfset description = description & '<p><font color=gray>Our Jewelry is all hand-made. Silver used is 92.5 sterling silver unless otherwise noted</font>'>
		  </cfif>
            ...
            <table   bgcolr="#ff7777" cellspacing="0" cellpadding="0" border="0"  align="center"  >
              ...
              <tr valign="top" >
                ...
                <table border="0" cellpadding="0" cellspacing="0" style="padding:1px" >
					...
					&nbsp;|<span style="background-color:##BBE100;color:black;">&nbsp; Item Id: #newitem# &nbsp;</span>|&nbsp;	<span   style="positio:absolute;top:8px;left:2px;z-index:1;background-color:transparent;" > 
...
This item is made on order (MOO) as it is currently out of stock. You may order it but it will take an additional 1 to 2 weeks to deliver.                       
                        </cfif>
                        #description#<p>
                        <b>Size:</b> 
...
inches
							  ...
							  (US sizing)<span class="error1"><br>[</span>Sizing rings adds two extra days to shipping.<span class="error1">]<br></span>
...
<cfif IsUserInRole("superadmin")>
		...
		Express: #dateformat(dateadd('d',8,now()),"d-mmm")#<br /></cfoutput></span></div>
                                <cfelse>
																<font color=red>WILL ARRIVE BY XMAS!</font>
																<!---
 ...
 <!--- <CFINPUT  type="text" SIZE="1"  id="quantity" Name="quantity" VALUE="#existqty#" REQUIRED="yes" Message="Please enter quantity" VALIDATE="integer" onclick="this.focus();" > --->
    ...
    <!---                        <img src=/images/addtocart.gif d="a2cb"  onClick="javascript:goProcess('add')" >   --->
                          ...
                          <!--- Angela --->
						 <cfif not len(width)> <INPUT TYPE="button" id="a2wl" style="width:100px;margin-top:14px;border:1px purple solid;" class="Green_button" onClick="javascript:goProcess('Add to Wish List')" VALUE="Add To Wish List" >
             ...
             <!--- End right side? --->
			  <!--- 4th column --->
                ...
                <cfquery name="groupitems" datasource="gemssql" >
					Select top #numbershow# itemID from itemsBygroup, items where itemsbygroup.itemid = newitem and GroupName='#findGroup.groupname#' <cfif category neq "All" >and category = '#category#'</cfif> and category <>'gems' and itemid <> #itemnumber# and inventory>0 and (items.status =0 or items.status=3) order by disporder
				</cfquery>
                <cfif groupitems.recordcount GT 0 and  len(findGroup.groupname)>
				 <cfoutput query="groupitems" ><a href=/detail.cfm?newitem=#itemid#><img src=/images/#category#/thumb/#itemID#.jpg border=0 width=75></a><br />
                    </cfoutput><cfoutput><a href=/shaped/#findGroup.groupname#-gemstone-#category#.cfm>More #findGroup.groupname# #category#</a>
					</cfoutput>
                <cfelse> <!--- not in any theme, show same stone items in different categories--->
				<cfquery name="othercatbestsellers" datasource="gemssql" >
				Select top 6 newitem, cat from items where subcat='#subcategory#' and cat <>'#details.cat#' and cat <> 'beads' and cat<>'gems' and (status=0 or status=3) and inventory>0 order by newitem 
				</cfquery>
