...
Select  orderbuy, disporder, newitem, CAT, subcat, datetaken,  NameID,     price  , saleprice, status, wholesaleprice, Description, inventory,
   ...
   buylink , case status when 3 then saleprice else price end as orderprice from items where storage = 'cmpet' and (inventory>#invstatus#)
...
<cfif isdefined("form.submit5")>
  ...
  <cfif isdefined("form.submit5")>
    ...
    <cfif isDefined("form.submit1")>
      <cfif not isDefined("form.cat1")>
        <cfif isDefined("form.cat")>
          where storage = 'cmpet' and (cat in (#PreserveSingleQuotes(form.cat)#)) 
        </cfif>
      </cfif>
      <cfelse>
...
Pet Jewelry and Pet Gemstone tags - Dog Pendants and Cat Pendants</TITLE>
<meta name="keywords" content="pet tags, dog tags, cat tags, dog pendant, cat pendant, dog gemstone pendant, cat gemstone pendant, gemstone, #subcat# pet jewelry, Pet Jewellery, pet gemstone jewelry, cz jewelry for pets">
<meta name="author" content="anup pandey" /><meta name="description" content='#subcat# gem stone pet tags and pet jewelry, for dog or cat pendants from gemstones. Silver jewelry for dogs, cats and other pets. <CFLOOP QUERY="Getlist" startrow="#start#"
...
<tr><td valign=top>
<img src='gymstones/images/gymstones_logo.jpg' width=200>&nbsp;&nbsp;<i>by Christy Martin</i></font></h2>
...
#session.mail#,#cgi.QUERY_STRING# .
  ...
  <cfset salestatus = 3>
      ...
      where (Storage ='cmpet'       <cfif advancedsearch is "">
        and storage='cmpet' and (CAT = '#category#' 
         )
      </cfif>
      <CFIF salestatus is 3>
        and (status = 3)
