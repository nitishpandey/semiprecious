...
<cfif cartid neq "">
  ...
  SELECT  cartstatus.cartid as cartid, indate, internalnote, email, itemid, CAT, supplier, thumblink, note, buyingrecord.optionid, quantity,  cartstatus.STATUS, buyingrecord.rate, price, saleprice, wholesaleprice, datebought, storage, 
  ...
  and quantity>0  <cfif showonly neq "">and note like '%#showonly#%'</cfif>   and cartstatus.cartid=BUYINGRECORD.CARTID and paymode <> 'null'  and items.inventory<0 and indate >'7/1/08'  order by #orderby# 
  </cfquery>
</cfif> 
...
<cfoutput>    <tr> 
      <td colspan="6">   Show only for: <cfoutput><a href=trackitem.cfm?showonly=harish>Harish</a> | <a href=trackitem.cfm?showonly=Acemat>ACEMAT</a> | | <a href=trackitem.cfm?showonly=Vijay>Vijay</a> | <a href=trackitem.cfm?showonly=Austin>AUSTIN</a> || ------------------ sort by --> <a href=trackitem.cfm?orderby=itemid>Item Number</a>  | <a href=trackitem.cfm?orderby=indate>Date of order</a></cfoutput>	</td>
...
<cfif storage is 'Angela'><b>WILL ARRIVE FROM OHIO</b><br></cfif><cfif supplier contains "OC"><b>Will Arrive Separately</b><br></cfif> #description# </td>
        <td  ><form name=#itemid# action="trackitem.cfm"><input type=hidden name=newitem value=#itemid#><input type=text name="newinv" size=1 value=#inventory#><input type=text name="store" size=10  value="<cfif q_ship.status is 'harish' and inventory lt 0>HARISH<cfelse>#storage#</cfif>"><input type=submit value="."></form>
		<form name="#itemid#note" action="trackitem.cfm"><input type="text" name="note" size=8 value="#note#"><input type="hidden" name="cartid" value="#cartid#"><input type=hidden name=item value=#itemid#><input type="submit" value="Ass."></form></td>
      </tr>
