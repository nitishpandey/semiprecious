...
<CFIF  isnumeric(itemnumber)>
	...
	--->
<cfif testing is "yes">
	  <CFQUERY datasource="gemssql" name="detailsinmemory" 		cachedwithin="#createTimespan(0,0,1,0)#">
		...
		<cfelse>
	  <CFQUERY datasource="gemssql" name="detailsinmemory" 		cachedwithin="#createTimespan(0,0,15,0)#">
		  Select grouping, itemnumber, size, weight, style, color, storage, imagelink, inventory,
			 orderonrequest, price, status, saleprice,clustercount,basecost, wholesaleprice, newitem, itemnumber as optcount,description,
			  NameID, newitem, subcat, subcat2, cat, morepics from 
		  items where cat<>'ornaments' and cat <>'bags' and (status =0 or status=3)
                 ...
                 <br /> 
                  ...
                  <div align="center" style="margin:4 0;">
                          <cfif not len(session.bulkbuyer.id)>
							<CFIF  status eq 3>
                              <b><font color="gray">Sale!</font>:#format(saleprice)#</b><br />
							<cfelse>
							<br />
                            </CFIF>
                          </cfif>
...
Ships in 6 Days <br><font color=gray size=-1>(50% of our items ship in 24 hrs)</font>
                                <cfelseif details.storage contains 'inpr'>
Arrives in 30 Days
<cfelse>																
																<font color=black>Ships in 24 hrs</font>
																<!---
		...
		Express: #dateformat(dateadd('d',5,now()),"d-mmm")#<br /></cfoutput>
                          ...
                          <CFIF (invent LTE 0) and not details.orderonrequest >
								Sold Out
								  <a href="/contactus.cfm?newitem=#newitem#&folder=#folder#" >Back Order / Request to Make</a>
                              ...
                              Only #invent# in stock
					              ...
					              <select id="quantity" name="quantity" >
										<cfloop from="1" to="#invent#" index="j">
											...
											<cfelse>
												<option value="#j#" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Buy #j#&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;</option>
											</cfif>
