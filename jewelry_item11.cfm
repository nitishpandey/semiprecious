...
<cfif cgi.server_name contains "wholesale-gemstone">
<script language=JAVASCRIPT src="/javascript/wholesaledetail2.js" type="text/javascript" ></script>
<cfelse>
...
<BODY BGCOLOR="white" link="black" topmargin='0' leftmargin="0" onLoad="javascript:ajaxcheck();location.href='#jewelry_top';"  >
		<cfif cgi.server_name contains "semiprecious.in" >
...
<div align="center" >
	...
	<span  style="text-decoration:none;position:relative;left:-40" ><cf_steps step="2" ></span>
	<!--- No category --->
	<!--- End no category --->
...
<tr valign=top>
  ...
  <td valign=top align="center" >
      ...
      <tr  valign="top" >
        <td   valign="top" align="Center" ><CFSET folder = #LCASE(category)#>
          ...
          <cfoutput>
            ...
            <table  width="756px" bgcolr="#ff7777" cellspacing="0" cellpadding="0" border="0" align="center"  >
              ...
              <tr valign="top" >
                 <!--- Main image --->
                <td width="560px" align="center" style="border:2px #CCC groove;" >
				  <cfoutput>
                ...
                <table border="0" cellpadding="0" cellspacing="0" style="padding:4px" >
	                <tr  valign="middle" >
	                	<td style="display:block;border: 0px ##DDD solid;position:relative;z-index:1;" valign="middle"  align="center" >
            ...
            <img id="mainimage" src ='/images/#folder#/#imagename#'  alt="unique #subcategory# #category# Jewelry"  border="0" >
			     ...
			     <cfif category eq 'beads'>
	                    <a class="login_link" href='/semiprecious-stone-beads.cfm?#session.filter#' >Back to Gallery</a>
                    <cfelseif isdefined("session.filter")>
                    	<cfif len(session.filter)>
	                    <a class="login_link" href='/gemstone_jewelry_gallery.cfm?#session.filter#' >Back to Gallery</a>
     					<cfelseif isdefined("session.gallery")>
	                    <a class="login_link" href='#session.gallery#' >Back to Gallery</a>
	                	</cfif>
                  </cfif>
    </DIV>          </cfoutput>
                ...
                <!---  <br style="clear:both" >--->
                 ...
                 <cfoutput>
				    <img  src ='/images/#folder#/<CFIF #NameID# is "" >#newitem#.jpg<CFELSE>#NameID#.jpg</CFIF>'  alt="unique #subcategory# #category# Jewelry"   height="70px" border="1" style="cursor:pointer" onMouseOver="swapMainImage('/images/#folder#/#imagename#');"  alt='unique #subcategory# #category# Jewelry' >
...
This is out of stock, we restock most items within 6 weeks                          
                        ...
                        <cfif details.size neq "" >
                          <cfif details.size is 0 and details.storage neq 'Angela'>
                            <i>Select size from drop down above 'Select button'</i>
                          </cfif>
...
inches
						...
						<cfif category is "bracelets" and details.itemnumber is "10">
							<i>Can select size from drop down above 'Select button'</i>
							</cfif>
...
Approx. number of beads calculate out to:                            #round(evaluate(16*25.4/details.size))#(this is auto calculated based on bead size entered and 16 inch strand length and can be wrong if lenght of strand is different or size of bead is not along the strand length)
    ...
    <!--- include file to display clustered items, by Anup--->
    	...
    	select top 12 styles.itemid , styles.cat from styles, items where items.newitem=itemid and inventory>0 and styleid =
        ...
        <cfset coll=coll+1>
        <td><a href=/jewelry_item.cfm?newitem=#itemid#><img src="/images/#cat#/thumb/#itemid#.jpg" border=0 width=100></td><cfif coll is 4></tr><tr><cfset coll=0></cfif>
        </cfoutput>
...
Ships in 24 hrs
                              ...
                              <cfif details.storage contains 'IN'>
                                <font color="red" ><b><span onMouseOver="var k = getDocumentElementById('del').innerHTML; getDocumentElementById('del2').innerHTML = k;"Delivery by:<br></b><div id="del">
		Regular: <cfoutput>#dateformat(dateadd('d',12,now()),"d-mmm")#<br>
		Priority: #dateformat(dateadd('d',10,now()),"d-mmm")#<br />
		Express: #dateformat(dateadd('d',8,now()),"d-mmm")#<br /><b></div></font></cfoutput>
                                <cfelse>
