...
<cfoutput>	 rtogd = #url.red#/#url.green# ;
	...
	hsv = rgbToHsv(#url.red#,#url.green#,#url.blue#);
	 ...
	 gtobd = 140/201;
	 btogd = 201/140;
	hsv = rgbToHsv(190,140,201);
    </cfif>
 
	var png2 = new PNG(data);
	    
	while(line = png2.readLine())
	{
		...
		//	 bd = h2d(min) + Math.floor((h2d(max)-h2d(min))); 
		     ...
		     rdn = Math.floor(rdn);
		     newhsv = rgbToHsv(rd,gd,bd);
		     //alert(newhsv.h);
		     newrgb = hsvToRgb(hsv.h,newhsv.s,newhsv.v);
		     //alert(newrgb.r);
...
</head><body onload="show(gravatar);"> 
	<div id="image" ></div> 
<div id="largeimage" ></div> 
<img id="nativeimage" src="" /> 
<form style="background-color:rgb(190,140,201);width:100px;position:absolute;left:500px;top:20px;" method="get" action=""> 
