					...
					for (j = 0; j < texture[i].length;j = j+5) 
					  {
						...
						textu = textu+","+ (px2.style.left = texture[i][j]) + ',' +	(px2.style.top = texture[i][j+1])
						+ ',' +	(px2.style.top = texture[i][j+2]) + ',' +	(px2.style.top = texture[i][j+3])+',' +( px2.style.width = texture[i][j+4]);
						px2.className = 'texture_pixel'+t_index;
						simg1.appendChild(px2);
                      }
				}	
							  simg1.style.zIndex = 4;   
