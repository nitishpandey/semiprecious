...
<cfset itemidlist = listappend(itemidlist,arraytolist(getitemid['itemid']))>
</cfloop><!--- <div   id="referenceitem" style="position:absolute;top:60px;left:120px;background-color:#white;padding:0px;z-index:2;">
...
select cat,newitem, description, <cfif session.bulkbuyer.id neq "">wholesaleprice as  price , wholesaleprice as saleprice <cfelse> price ,  saleprice</cfif> , status, inventory from items where newitem  in   (#itemidlist#) and (status = 3 or status  = 0) order by cat
</cfquery>
</cfif>
<CFIF not ISDEFINED("gETLIst.recordcount")>
<div align="center"><body topmargin="0" background="/images/bg.jpg">
<cfinclude template="header.cfm">Sorry, NO ITEM FOUND!.
Try using our search tools on the home page. You can search typing in your description, you can search by product categories and then by stones/silver, cost, color etc.
...
<BODY  leftmargin="auto"  BGCOLOR="white" text='black' link="black" onLoad="MM_showHideLayers('referenceitem','','hide');" vlink='#006600' topmargin='0' >
<div align="center">
<CFINCLUDE TEMPLATE="headerforlist.cfm"> 
<cfoutput>
<div  style="width:800px;" align="left"><h3><cfif  session.bulkbuyer.id neq "">Wholesale Shopper: #session.name#
<cfelseif session.mail neq "">Member: #session.mail#<cfelse><cf_steps step="1" /></cfif></h3></div>
<cfset displaycolumns = 2>
<cfset displayrows = 5>
...
<CFSET currentrow= start>
	                  ...
	                  <cfoutput group="cat" QUERY="Getlist"		startrow="#start#"		maxrows="#enddisplay#">
					  				  <cfif columns NEQ 1> 
			  <td colspan="#evaluate(2*(displaycolumns - columns +1))#" class="rowb">&nbsp;</td></tr>
              
			  </cfif><CFSET columns=1>
<tr><td class="biggreenwhite" colspan="#evaluate(2*displaycolumns)#" >&nbsp;#cat# of  same #type#</td></tr>
					  <cfoutput>
						   <CFIF COLUMNS eq 1>
            			  <tr>
			  </CFIF>
       ...
       <span  align="justify" class="listdescr">#description#</span>
         ...
         <cfelse>
		  ...
		  <tr><td colspan="#evaluate(2*displaycolumns)#" class="rowb">
		   <table width="705"  BORDER=0  class="rowt">
            <tr > 
