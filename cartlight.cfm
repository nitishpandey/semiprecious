...
<cfset session.bulkbuyer.minamt=50> <!--- special email sent minimum reduced for wholesale --->
...
if(inString=='Proceed to Checkout')
{
	...
	<cfelse>
	self.document.cartform.action="https://www.semiprecious.com/confirmationlight.cfm?cartid=<cfoutput>#session.cartid#</cfoutput>";
			//BrowserDetect.init();
  ...
  <TH width="12%">Amt (#session.currency#)</TH>
    ...
    <cfset rowb = "rowb" & (j mod 2)/>
          <CFQUERY NAME="qprice" DATASOURCE="gemssql" timeout="40"    >
           SELECT
                <cfif session.cartitem[j][5]> options.description as optsdesc, </cfif>  storage, subcat+ ' ' + cat as description,clustercount,status,storage,thumblink FROM Items 
              ...
              <tr>
                ...
                <td  align="left">
							...
							<select name="SpecialHandling" >  
									...
									<option   value="2.00"> 'Quick Ship' Items only: Priority Mail (1-2 Day Delivery) $<cfoutput>#evaluate(shipping+2)#</cfoutput></option> 
									 	<option   value="10.00"> 'Quick Ship' Items only: Express (1-2 Day Delivery) $<cfoutput>#evaluate(shipping+10)#</cfoutput></option> 
									</cfif>
							  </select><br />
...
<span id="siteseal"><script type="text/javascript" src="https://seal.godaddy.com/getSeal?sealID=hF33DTtyM65nCwzDN0SwqGGgTQPxn3o4Ym0uzkuDY7OQkc0HNahexOlJ"></script>
--->
		   <span style="color:gray;">Coupon or Gift Code (If Any):</td><td> <input name="couponcode" type="text" class="detail" value="" size="12" maxlength="18"><br />	    
</td>
      </tr>
			      <tr>
